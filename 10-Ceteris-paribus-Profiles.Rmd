...
To plot CP profiles for categorical variables, we have got to add the `variable_type = "categorical"` argument to the `plot()` function. In the code below, we use argument `variables` to indicate that we want to create plots for *class* and *embarked* variables. The resulting plot is shown in Figure \@ref(fig:titanicCeterisProfile01B). 
...
```{python,  eval = FALSE}
cp_johnny = titanic_rf_exp.predict_profile(johnny_d)
cp_johnny.result
```
