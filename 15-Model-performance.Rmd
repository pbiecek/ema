...
If we assign the value of 1 to success and 0 to failure, it is possible to use MSE, RMSE, and MAD, as defined in Equations \@ref(eq:MSE), \@ref(eq:RMSE), \@ref(eq:MAD), respectively, as a GoF measure. In fact, the MSE obtained in that way is equivalent to the Brier score, which can be also expressed as 
$$
...
Figure \@ref(fig:examplePRC) presents the precision-recall curve (left-hand-side panel) and lift chart (right-hand-side panel) for the random forest model. 
