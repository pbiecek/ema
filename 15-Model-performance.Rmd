...
(ref:exampleROCDesc) Receiver Operating Characteristic curve for the random-forest model for the Titanic dataset. The Gini coefficient can be calculated as 2 x area between the ROC curve and the diagonal (this area is highlighted). [TOMASZ: STATIC FIGURE. CHECK IF CORRECT.]

```{r exampleROC, fig.width=5, fig.height=5, echo=FALSE, fig.cap='(ref:exampleROCDesc)', out.width = '70%', fig.align='center'}
...
Let us consider the linear-regression model `apartments_lm_v5` (see Section \@ref(model-Apartments-lr)) and the random-forest model `apartments_rf_v5` (see Section \@ref(model-Apartments-rf)) for the apartment-prices  data (see Section \@ref(ApartmentDataset)). Recall that, for these data, the dependent variable, the price per square meter, is continuous. Hence, we can use the performance measures presented in Section \@ref(modelPerformanceMethodCont). In particular, we consider MSE and RMSE. The values of the two measures, computed for the `apartments_test` testing dataset (see Section \@ref(predictionsApartments)) for the two models, are presented below. 
...
## rmse   282.952 rmse
```
Both MSE and RMSE indicate that, overall, the random-forest model performs better than the linear-regression model.
...
```{r titanicROC, warning=FALSE, message=FALSE, echo=FALSE, fig.width=5, fig.height=5, out.width = '70%', fig.cap='(ref:titanicROCDesc)', fig.align='center'}
library("DALEX")
library("rms")
library("randomForest")
...
```{r titanicLift, warning=FALSE, message=FALSE, echo=FALSE, fig.width=5, fig.height=5, out.width = '70%', fig.cap='(ref:titanicLiftDesc)', fig.align='center'}
...
For illustration purposes, we use the random-forest model `titanic_rf_v6` (see Section \@ref(model-titanic-rf)) and the logistic-regression model `titanic_lmr_v6` (see Section \@ref(model-titanic-lmr)) for the Titanic data (see Section \@ref(TitanicDataset)). Consequently, the `DALEX` functions are applied in the context of a binary classification problem. However, the same functions can be used for, for instance, linear-regression models.

To illustrate the use of the functions, we first retrieve the `titanic_lmr_v6` and `titanic_rf_v6` model-objects via the `archivist` hooks, as listed in Section \@ref(ListOfModelsTitanic). We also retrieve the version of the `titanic` data with imputed missing values.
...
library("rms")
library("DALEX")
explain_lmr <- explain(model = titanic_lmr_v6, data = titanic_imputed[, -9],
    y = titanic_imputed$survived == "yes", label = "Logistic Regression", verbose = FALSE)
explain_lmr$model_info$type = "classification"
explain_rf <- explain(model = titanic_rf_v6,  data = titanic_imputed[, -9],
...
```{r titanicMEexamples, warning=FALSE, message=FALSE, eval=FALSE, fig.cap='(ref:titanicMEexamplesDesc)', fig.width=9, fig.height=5, fig.align='center'}
...
The resulting graph (not shown) contains the plots presented in Figures \@ref(fig:titanicROC) and \@ref(fig:titanicLift). 
