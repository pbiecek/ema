...
For illustration purposes, we use the random-forest model `titanic_rf_v6` (see Section \@ref(model-titanic-rf) and the logistic regression model `titanic_lmr_v6` (see Section \@ref(model-titanic-lmr)) and the random-forest model `titanic_rf_v6` (see Section \@ref(model-titanic-rf)) for the Titanic data (see Section \@ref(TitanicDataset)). Consequently, the  functions from the `auditor` package are applied in the context of a binary classification problem. However, the same functions can be used for, e.g., linear regression problems.
