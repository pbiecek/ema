...
[TOMASZ: THIS FIGURE MAKES NO SENSE HERE - IT DOES NOT EVEN ILLUSTRATE THE CALCULATION OF rmse. MOREOVER, WE DO SHOW IT AS A PART OF A FIRGUE IN SEC 15.4.]
...
Assume that a testing dataset is available. In that case, we can use model $f()$, obtained by fitting the model to training data, to predict the values of the dependent variable observed in the testing dataset. Subsequently, we can compute MSE as in \@ref(eq:MSE) to obtain the mean squared-prediction-error (MSPE) as a GoP measure [@Kutner2005]. By taking the square root of MSPE, we get the root-mean-squared-prediction-error (RMSPE).  
...
Table: (\#tab:confMat) Confusion table for a classification model.
...
A variant of ROC curve based on precison and recall is a called a precision-recall curve. Figure \@ref(fig:examplePRC) the curve for the random-forest model for the Titanic dataset. 
...
<!---
[TOMASZ: REDUNDNAT, WE DO SAY IT IN THE INTRODUCTION SETTING]
...
(ref:prepareMPBoxplotExDesc) Box plot for the absolute values of residuals for the linear-regression and random-forest models for the apartment-prices data. The red dot indicates the RMSE.
...
First, we take a look at the random-forest model. We will illustrate the "confusion table" by using threshold $C$ equal to 0.5, i.e., we will classify passengers as "survivors" and "non-survivors" depending whether their model-predicted probability of survival was larger than 50\% or not, respectively. Table  \@ref(tab:confMatRF) presents the resulting table.  

Table: (\#tab:confMatRF) Confusion table for the random-forest model for the Titanic data. Predicted survival status is equal to "TRUE" if the model-predicted probability of survival is larger than 50\%. 

|                          |Actual survival: TRUE|Actual survival: FALSE| Total |
|--------------------------|---------------------|----------------------|-------|
| Predicted survival: TRUE |                  454|                    60|    514|
| Predicted survival: FALSE|                  257|                  1436|   1693|
| Total                    |                  711|                  1496|   2207|
...
library(caret)
cm_rf <- confusionMatrix(data=as.factor(predlab_rf), reference=as.factor(eva_rf$residuals$observed), 
...
library(caret)
cm_lr <- confusionMatrix(data=as.factor(predlab_lr), reference=as.factor(eva_lr$residuals$observed), 
 positive = "TRUE", dnn = c("Predicted status","True status"))
# cm_lr
## Confusion Matrix and Statistics
## 
...
(ref:exampleROCDesc) Receiver Operating Characteristic curve for the random-forest model for the Titanic dataset. The Gini coefficient can be calculated as 2 x area between the ROC curve and the diagonal (this area is highlighted). The AUC coefficient is defined as an area under the ROC curve.
...
Figure \@ref(fig:exampleROC) presents the precision-recall curve for the random-forest model. 
...
Figure \@ref(fig:exampleROC) presents the lift chart for the random-forest model. 
...
Table: (\#tab:confMatLR) Confusion table for the logisitc-regression model for the Titanic data. Predicted survival status is equal to "TRUE" if the model-predicted probability of survival is larger than 50\%. 

|                          |Actual survival: TRUE|Actual survival: FALSE| Total |
|--------------------------|---------------------|----------------------|-------|
| Predicted survival: TRUE |                  416|                   137|    653|
| Predicted survival: FALSE|                  295|                  1359|   1654|
| Total                    |                  711|                  1496|   2207|

Figure \@ref(fig:titanicROC) presents ROC curves for both the logistic-regression and the random-forest model. The curve for the random-forest model lies above the one for the logistic-regression model for the majority of the cut-offs $C$, except for the very high values of of cut-off $C$. AUC for the logistic-regression model is equal to 0.8174 and is smaller than for the random-forest model. 
...
In this section, we present model-performance measures as implemented in the `DALEX` package for R. The package covers the most often used measures and methods presented in this chapter. More advanced measures of performance are available in the `auditor` R package [@R-auditor]. Note that there are also other R packages that offer similar functionality. These include, for instance, packages `mlr` [@mlr], `caret` [@caret], `tidymodels` [@tidymodels], and `ROCR` [@ROCR].
...
The resulting graph is shown in Figure \@ref(fig:titanicMEexamples). Combined with the plot of ROC curves and the lift charts presented in Figures \@ref(fig:titanicROC) and \@ref(fig:titanicLift), respectively, it provides additional insight into the comparison of performance of the two models. 
...
To calculate selected mesures of the overall performance, we use the `model_performance()` method. In the syntax below, we apply the `model_type` argument to indicate that we deal with a classification problem, and the `cutoff` argument to specify the cutoff value equal to 0.5. [TOMASZ: DIFFERENT VALUES THAN IN R. COMMENT? RESULTS AS A FIGURE?] 
...
The resulting object can be visualised by applying the `plot()` method. Figure \@ref(fig:examplePythonMP2) presents the created plot. [TOMASZ: CDF OF RESIDUALS? WHY NOT A ROC CUVRE? IF POSSIBLE, HOW TO GET IT?]  

```{python,  eval = FALSE}
mp_rf.plot()
```

(ref:examplePythonMP2Desc) The default plot obtained by the generic `plot()` method in Python for the random-forst model for the Titanic dataset.  [TOMASZ: CDF OF RESIDUALS IS NOT VERY USEFUL. WHY NOT A ROC CURVE?]

```{r examplePythonMP2, echo=FALSE, out.width = '100%', fig.align='center', fig.cap='(ref:examplePythonMP2Desc)'}
