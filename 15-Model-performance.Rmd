...
|-----------------------|-------------------------|---------------------------------|-----------|
|  $f(x) > C$           | True Positive:          | False Positive (type I error):  |           |
|  Predicted: `success` |      $TP_C$             |   $FP_C$                        | $P_C$     |
|-----------------------|-------------------------|---------------------------------|-----------|
|  $f(x) \leq C$        | False Negative  (type II error):  | True Negative:        |           |
|  Predicted: `failure` |      $FN_C$             |   $TN_C$                        | $N_C$     |
|-----------------------|-------------------------|---------------------------------|-----------|
| Total                 | S                       | F                               | $n$       |
...
All the measures depend on the choice of cut-off $C$. To assess the form and the strength of dependence, a common approach is to construct the Receiver Operating Characteristic (ROC) curve. The curve plots the $Sensitivity_C$ in function of $1-Specificity_C$ for all possible, ordered values of $C$. Figure \@ref(fig:exampleROC) presents the ROC curve for the random-forest model for the Titanic dataset (see Section \@ref(model-titanic-rf)). Note that the curve indicates an inverse relationship between sensitivity and specifcity: by increasing one measure, the other is decreased.     

```{r exampleROC, fig.width=5, fig.height=5, echo=FALSE, fig.cap="ROC curve for the random-forest model for the Titanic dataset. The Gini coefficient can be calculated as 2 x area between the ROC curve and the diagonal (this area is highlighted).", out.width = '80%', fig.align='center'}
...
$$
Lift  = \frac{\frac{TP_C}{P_C}}{\frac{S}{n}} = \frac{n Precision_C}{S}.
$$
Note that $S/n$ can be seen as the estimated probability of a correct prediction of a success for random guessing. On the other hand, $TP_C/P_C$ is the estimated  probability of a correct prediction a succcess given that the model predicts a success. Hence, informally speaking, the lift indicates how many more (or less) times the model does better in predicting success than random quessing. As other measures, the lift depends on the choice of cut-off $C$. The plot of the lift as a function of $P_C$ is called the lift chart.
...
In machine-learning world, this approach is often termed ''micro-averaging'' (hence subscript $\mu$ for ''micro'' in $Precision_{\mu}$ in \@ref(eq:precmicro)). Note that, for accuracy, this computation still leads to \@ref(eq:accmacro). The measures computed in that way favor classes with larger numbers of observations.
...
explainer_apartments_lr <- explain(explainer_apartments_lr$model,
                                   data = explainer_apartments_lr$data,
                                   y = explainer_apartments_lr$y)
...
explainer_apartments_rf <- explain(explainer_apartments_rf$model,
                                   data = explainer_apartments_rf$data,
                                   y = explainer_apartments_rf$y)
...
explainer_titanic_rf <- explain(explainer_titanic_rf$model,
                               data = explainer_titanic_rf$data,
                               y = explainer_titanic_rf$y,
                               type = "classification")

explainer_titanic_lr <- archivist:: aread("pbiecek/models/42d51")
explainer_titanic_lr <- explain(explainer_titanic_lr$model,
                               data = explainer_titanic_lr$data,
                               y = explainer_titanic_lr$y,
                               type = "classification")

DALEX::model_performance(explainer_titanic_rf)
DALEX::model_performance(explainer_titanic_lr)
```
...
```{r titanicROC, warning=FALSE, message=FALSE, echo=FALSE, fig.width=5, fig.height=5, fig.cap="ROC curves for the  random-forest model and the logistic regression model for the Titanic dataset."}
library("DALEX")
library("randomForest")

# plot results
explain_titanic_rf <- explain(explain_titanic_rf$model, 
                              explain_titanic_rf$data, 
                              y = explain_titanic_rf$y, 
                              type = "classification",
                              verbose = FALSE)
explain_titanic_lr <- explain(explain_titanic_lmr$model,
                              explain_titanic_lmr$data, 
                              y = explain_titanic_lmr$y, 
                              type = "classification",
                              verbose = FALSE)

eva_rf <- DALEX::model_performance(explain_titanic_rf)
eva_lr <- DALEX::model_performance(explain_titanic_lr)
plot(eva_rf, eva_lr, geom = "roc")
```

Figure \@ref(fig:titanicLift) presents lift curves for both models. Also in this case the curve for the random-forest suggests a better performance than for the logistic regression model, except for the very high values of cut-ff $C$.


```{r titanicLift, warning=FALSE, message=FALSE, echo=FALSE, fig.width=5, fig.height=5, fig.cap="Lift curves for the  random-forest model and the logistic regression model for the Titanic dataset."}
plot(eva_rf, eva_lr, geom = "lift")
```

```{r titanicGain, warning=FALSE, message=FALSE, echo=FALSE, fig.width=5, fig.height=5, fig.cap="Cummulative gain chart for the  random-forest model and the logistic regression model for the Titanic dataset."}
...
model_performance(explain_titanic_lr)
```
...
```{r titanicMEexamples, warning=FALSE, message=FALSE, fig.width=5, fig.height=5}
eva_rf <- model_performance(explain_titanic_rf)
eva_lr <- model_performance(explain_titanic_lr)
...
```{r titanicBoxplots, fig.cap="Boxplots for residuals for two models on Titanic dataset.",  warning=FALSE, message=FALSE, fig.width=5, fig.height=3}
