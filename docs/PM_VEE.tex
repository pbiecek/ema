...
\ref{RCodeSnippets}, we provide a short description of R tools and
...
Chapters \ref{ceterisParibus}-\ref{CompareInstance}. In Chapters
\ref{ceterisParibus}-\ref{ceterisParibusWankardu}, methods based on
...
Chapter \ref{ceterisParibusWankardu} describes local-fidelity plots that
are useful to investigate the sources of a poor prediction for a
particular single observation.
...
black-box model by a simpler, white-box one. In paricular, in the
chapter, the Local Interpretable Model-Agnostic Explanations (LIME)
method \citep{lime} is discussed.

The final chapter of the first part, Chapter \ref{CompareInstance},
...
\ref{ModelLevelExplanations}-\ref{conceptDrift}.
...
\hypertarget{RCodeSnippets}{%
\section{Do-it-yourself With R}\label{RCodeSnippets}}
...
\hypertarget{PythonCodeSnippets}{%
\section{Do-it-yourself With Python}\label{PythonCodeSnippets}}
...
\item
  ...
  \ref{ceterisParibus}-\ref{ceterisParibusWankardu} introduce the CP
  profiles and methods based on them.\\
\item
...
\item
  ...
  black-box model, we may approximate it with a simpler white-box model
...
discussed in the corresponding chapters. Chapter \ref{CompareInstance}
offers a comparison of the methods.
...
Special cases are interactions, which require the use of two-dimensional
CP profiles that are more complex than one-dimensional ones. Also, in
case of a model with hundreds or thousands of variables, the number of
plots to inspect may be daunting. Finally, while barplots allow
visualization of CP profiles for factors (categorical explanatory
...
\texttt{select\_neighbours()} function from the
\texttt{ingredients\textquotesingle{}\ package.\ It\ returns}n\texttt{observations\ (by\ default,\ 20)\ most\ similar\ to\ the\ observation\ of\ interest\ according\ to\ the}distance`
measure (by default, the Gower distance is used).
...
In the Section \ref{variableAttributionMethods} we show a procedure that
attributes parts of model prediction to input features. We also show
that in the presence of interactions attributions depend on the feature
ordering. One solution to this problem is introduced in section
\ref{breakDown} - just find an ordering that put most important features
to the front. Other solution is introduced in the Section
...
random orderings for Titanic dataset. As we see there are differences,
most striking are that depending on the order features like
\texttt{fare} or \texttt{class} may have positive or negative effect.
...
attributions for a selected variable.
...
Sections \ref{modelAgnosticAttribution}, \ref{breakDown} and
\ref{shapley} lead to same variable contributions.
...
\NormalTok{explain_titanic_rf <-}\StringTok{ }\KeywordTok{explain}\NormalTok{(}\DataTypeTok{model =}\NormalTok{ titanic_rf_v6, }
                              \DataTypeTok{data =}\NormalTok{ titanic[,}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{4}\NormalTok{,}\DecValTok{6}\OperatorTok{:}\DecValTok{8}\NormalTok{)],}
                              \DataTypeTok{y =}\NormalTok{ titanic}\OperatorTok{$}\NormalTok{survived }\OperatorTok{==}\StringTok{ "yes"}\NormalTok{, }
                              \DataTypeTok{label =} \StringTok{"Random Forest v6"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{shap_johny <-}\StringTok{ }\KeywordTok{shap}\NormalTok{(explain_titanic_rf, johny_d, }\DataTypeTok{B =} \DecValTok{25}\NormalTok{)}
...
\textbf{Fitting a white box model in this neighborhood}
...
\NormalTok{iml_white_box =}\StringTok{ }\NormalTok{LocalModel}\OperatorTok{$}\KeywordTok{new}\NormalTok{(iml_rf, }\DataTypeTok{x.interest =}\NormalTok{ johny_d, }\DataTypeTok{k =} \DecValTok{6}\NormalTok{)}
\NormalTok{iml_white_box}
...
\KeywordTok{plot}\NormalTok{(iml_white_box) }
\end{Highlighting}
\end{Shaded}
...
\hypertarget{CompareInstance}{%
\section{Comparision of Instance-level
Explainers}\label{CompareInstance}}
...
\hypertarget{ModelLevelExplanations}{%
\section{Model level explanations}\label{ModelLevelExplanations}}
