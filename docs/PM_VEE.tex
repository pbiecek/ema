...
The dataset is available in the \texttt{DALEX} package \citep{R-DALEX}. It contains 1000 observations (apartments) and six variables:
...
\NormalTok{explain_apartments_lm_v5 <-}\StringTok{ }\KeywordTok{explain}\NormalTok{(}\DataTypeTok{model =}\NormalTok{ apartments_lm_v5, }
                                 ...
                                 \DataTypeTok{y =}\NormalTok{ apartments}\OperatorTok{$}\NormalTok{m2.price,}
...
\end{minipage} & \begin{minipage}[t]{0.25\columnwidth}\raggedright
Get the model: \texttt{archivist::\ aread("pbiecek/models/55f19")}. Get the explainer: \texttt{archivist::\ aread("pbiecek/models/b0734")}\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.21\columnwidth}\raggedright
...
Note that there are also other R packages that offer similar functionality, like \texttt{condvis} \citep{JSSv081i05}, \texttt{pdp} \citep{pdpRPackage}, \texttt{ICEbox} \citep{ICEboxRPackage}, \texttt{ALEPlot} \citep{ALEPlotRPackage}, \texttt{iml} \citep{imlRPackage}.
...
In this section, we show how to use the R package \texttt{ingredients} \citep{R-ingredients} to construct local-fidelity plots. More details and examples can be found at \texttt{https://modeloriented.github.io/ingredients/}.
...
\{\centering \includegraphics[width=0.7\linewidth]{PM_VEE_files/figure-latex/BDjohnyExample-1}
...
\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-40-1.pdf}
...
\includegraphics{PM_VEE_files/figure-latex/titanicROCexamples-1.pdf}
...
\includegraphics{PM_VEE_files/figure-latex/titanicROCexamples-2.pdf}
...
\NormalTok{rf_explainer <-}\StringTok{ }\KeywordTok{explain}\NormalTok{(rf_model, }\DataTypeTok{data =}\NormalTok{ dragons, }\DataTypeTok{y =}\NormalTok{ dragons}\OperatorTok{$}\NormalTok{life_length,}
...
##   -> predicted values  :  numerical, min =  625.1628 , mean =  1370.024 , max =  3282.816  
##   -> residual function :  difference between y and yhat (  default  )
##   -> residuals         :  numerical, min =  -145.9873 , mean =  0.9614219 , max =  699.6195  
##   A new explainer has been created!
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lm_explainer <-}\StringTok{ }\KeywordTok{explain}\NormalTok{(lm_model, }\DataTypeTok{data =}\NormalTok{ dragons, }\DataTypeTok{y =}\NormalTok{ dragons}\OperatorTok{$}\NormalTok{life_length, }
          \DataTypeTok{colorize =} \OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{svm_explainer <-}\StringTok{ }\KeywordTok{explain}\NormalTok{(svm_model, }\DataTypeTok{data =}\NormalTok{ dragons, }\DataTypeTok{y =}\NormalTok{ dragons}\OperatorTok{$}\NormalTok{life_length, }
          \DataTypeTok{colorize =} \OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{rf_model <-}\StringTok{ }\KeywordTok{ranger}\NormalTok{(life_length }\OperatorTok{~}\StringTok{ }\NormalTok{., }\DataTypeTok{data =}\NormalTok{ dragons)}
\NormalTok{rf_au <-}\StringTok{ }\KeywordTok{explain}\NormalTok{(rf_model, }\DataTypeTok{data =}\NormalTok{ dragons, }\DataTypeTok{y =}\NormalTok{ dragons}\OperatorTok{$}\NormalTok{life_length)}
...
\NormalTok{lm_mr <-}\StringTok{ }\KeywordTok{model_residual}\NormalTok{(rf_au)}
\KeywordTok{plot}\NormalTok{(lm_mr, }\DataTypeTok{type =} \StringTok{"prediction"}\NormalTok{, }\DataTypeTok{abline =} \OtherTok{TRUE}\NormalTok{)}

\KeywordTok{plot}\NormalTok{(lm_mr,  }\DataTypeTok{variable =} \StringTok{"height"}\NormalTok{, }\DataTypeTok{type =} \StringTok{"prediction"}\NormalTok{, }\DataTypeTok{abline =} \OtherTok{TRUE}\NormalTok{)}

\KeywordTok{plot_residual}\NormalTok{(lm_mr, }\DataTypeTok{variable =} \StringTok{"_y_hat_"}\NormalTok{, }\DataTypeTok{nlabel =} \DecValTok{10}\NormalTok{)}

\KeywordTok{plot}\NormalTok{(lm_mr, }\DataTypeTok{type =} \StringTok{"residual_density"}\NormalTok{)}

\KeywordTok{plot}\NormalTok{(lm_mr, }\DataTypeTok{type =} \StringTok{"residual_boxplot"}\NormalTok{)}

\CommentTok{# alternatives:}
\CommentTok{# plot_prediction(rf_mr, lm_mr, abline = TRUE)}
\CommentTok{# plot_prediction(rf_mr, lm_mr, variable = "life_length")}

\KeywordTok{plot}\NormalTok{(rf_mr, lm_mr, }\DataTypeTok{variable =} \StringTok{"scars"}\NormalTok{, }\DataTypeTok{type =} \StringTok{"prediction"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
