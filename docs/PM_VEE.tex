...
\item
  Model's performance: how \texttt{good\textquotesingle{}\textquotesingle{}\ is\ the\ model?\ Is\ one\ model}better'' than another?
\item
...
We focus on the method described in more detail in \citep{variableImportancePermutations}. The main idea is to measure how much the model fit decreases if the effect of a selected explanatory variable or of a group of variables is removed. The effect is removed by means of perturbations like resampling from an empirical distribution of just permutation of the values of the variable. {[}TOMASZ: CORRECT?{]}
