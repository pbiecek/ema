...
## ok         533 1250      438   0.4371905
## promoted   204  385     2182   0.2125586
\end{verbatim}
...
## randomForest.fired: * salary = 2              -0.254
## randomForest.fired: * age = 58                 0.075
## randomForest.fired: * gender = male            0.279
## randomForest.fired: prediction                 0.774
...
## randomForest.ok: * age = 58                   -0.073
## randomForest.ok: * gender = male              -0.284
## randomForest.ok: prediction                    0.216
...
## randomForest.promoted: * age = 58             -0.002
## randomForest.promoted: * gender = male         0.005
## randomForest.promoted: prediction              0.010
## baseline:  0
\end{verbatim}
...
## Confusion matrix:
##          fired   ok promoted class.error
## fired     2278  382      195   0.2021016
## ok         542 1241      438   0.4412427
## promoted   200  390     2181   0.2129195
\end{verbatim}
...
## randomForest.fired: baseline                         0.376
## randomForest.fired: * hours = 42                     0.240
## randomForest.fired: * salary = 2                    -0.214
## randomForest.fired: * age:gender = 58:male           0.419
## randomForest.fired: * evaluation = 2                -0.019
## randomForest.fired: prediction                       0.802
## randomForest.ok: baseline                            0.275
## randomForest.ok: * evaluation = 2                    0.135
## randomForest.ok: * salary = 2                        0.158
## randomForest.ok: * age:gender = 58:male             -0.278
## randomForest.ok: * hours = 42                       -0.097
## randomForest.ok: prediction                          0.194
...
importance. There is many possible applications of such methods, for
example:
...
\item
  ...
  fitting. Removal of the noise shall lead to a better model.
\item
...
There are many methods for feature importance. In general we may divide
them into two groups, methods that are model specific and methods that
are model agnostic.

Methods like random forest, extreme gradient boosting, linear models and
many others have their own ways to assess feature importance. But in
this chapter we will focus on methods that are model agnostic. The may
reason for that is
...
of feature effect, i.e.~the way how a feature is linked with model
response. There is many possible applications of such methods, for
example:
...
\item
  ...
  in a way to better utilised the feature in a simpler model.
\item
  understanding how the model utilise a feature may be used as a
...
\NormalTok{rf_model <-}\StringTok{ }\KeywordTok{randomForest}\NormalTok{(Survived }\OperatorTok{~}\StringTok{ }\NormalTok{Pclass }\OperatorTok{+}\StringTok{ }\NormalTok{Sex }\OperatorTok{+}\StringTok{ }\NormalTok{Age }\OperatorTok{+}\StringTok{ }\NormalTok{SibSp }\OperatorTok{+}\StringTok{ }\NormalTok{Parch }\OperatorTok{+}\StringTok{ }\NormalTok{Fare }\OperatorTok{+}\StringTok{ }\NormalTok{Embarked, }
                         \DataTypeTok{data =}\NormalTok{ titanic_small)}
\NormalTok{predict_fuction <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(m,x) }\KeywordTok{predict}\NormalTok{(m, x, }\DataTypeTok{type=}\StringTok{"prob"}\NormalTok{)[,}\DecValTok{2}\NormalTok{]}
\NormalTok{rf_explain <-}\StringTok{ }\KeywordTok{explain}\NormalTok{(rf_model, }\DataTypeTok{data =}\NormalTok{ titanic_small, }
                      \DataTypeTok{y =}\NormalTok{ titanic_small}\OperatorTok{$}\NormalTok{Survived }\OperatorTok{==}\StringTok{ "1"}\NormalTok{, }\DataTypeTok{label =} \StringTok{"RF"}\NormalTok{,}
                      \DataTypeTok{predict_function =}\NormalTok{ predict_fuction)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{cp_}\DecValTok{25}\NormalTok{ <-}\StringTok{ }\KeywordTok{ceteris_paribus}\NormalTok{(rf_explain, }
                       \DataTypeTok{observations =}\NormalTok{ titanic_small[}\KeywordTok{sample}\NormalTok{(}\KeywordTok{nrow}\NormalTok{(titanic_small),}\DecValTok{25}\NormalTok{), ], }
                       \DataTypeTok{variables =} \StringTok{"Age"}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(cp_}\DecValTok{25}\NormalTok{, }\DataTypeTok{show_observations =} \OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(cp_}\DecValTok{25}\NormalTok{, }\DataTypeTok{show_observations =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{alpha =} \FloatTok{0.1}\NormalTok{) }\OperatorTok{+}\StringTok{ }
\KeywordTok{ceteris_paribus_layer}\NormalTok{(cp_}\DecValTok{25}\NormalTok{, }\DataTypeTok{show_observations =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{aggregate_profiles =}\NormalTok{ mean, }\DataTypeTok{alpha =} \DecValTok{1}\NormalTok{, }\DataTypeTok{size =} \DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-48-2.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cp_}\DecValTok{100}\NormalTok{ <-}\StringTok{ }\KeywordTok{ceteris_paribus}\NormalTok{(rf_explain, }
                        \DataTypeTok{observations =}\NormalTok{ titanic_small[}\KeywordTok{sample}\NormalTok{(}\KeywordTok{nrow}\NormalTok{(titanic_small),}\DecValTok{100}\NormalTok{), ], }
                        \DataTypeTok{variables =} \KeywordTok{c}\NormalTok{(}\StringTok{"Age"}\NormalTok{,}\StringTok{"Sex"}\NormalTok{))}
\KeywordTok{plot}\NormalTok{(cp_}\DecValTok{100}\NormalTok{, }\DataTypeTok{color =} \StringTok{"Sex"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-48-3.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cp_male <-}\StringTok{ }\KeywordTok{ceteris_paribus}\NormalTok{(rf_explain, }
                         \DataTypeTok{observations =}\NormalTok{ titanic_small[titanic_small}\OperatorTok{$}\NormalTok{Sex }\OperatorTok{==}\StringTok{ "male"}\NormalTok{,], }
                         \DataTypeTok{variables =} \KeywordTok{c}\NormalTok{(}\StringTok{"Age"}\NormalTok{, }\StringTok{"Sex"}\NormalTok{))}
\NormalTok{cp_female <-}\StringTok{ }\KeywordTok{ceteris_paribus}\NormalTok{(rf_explain, }
                           \DataTypeTok{observations =}\NormalTok{ titanic_small[titanic_small}\OperatorTok{$}\NormalTok{Sex }\OperatorTok{==}\StringTok{ "female"}\NormalTok{,], }
                           \DataTypeTok{variables =} \KeywordTok{c}\NormalTok{(}\StringTok{"Age"}\NormalTok{, }\StringTok{"Sex"}\NormalTok{))}

\KeywordTok{plot}\NormalTok{(cp_male, }\DataTypeTok{show_observations =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{aggregate_profiles =}\NormalTok{ mean, }\DataTypeTok{color =} \StringTok{"red3"}\NormalTok{) }\OperatorTok{+}
\KeywordTok{ceteris_paribus_layer}\NormalTok{(cp_female, }\DataTypeTok{show_observations =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{aggregate_profiles =}\NormalTok{ mean, }\DataTypeTok{color=}\StringTok{"blue3"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-48-4.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(}\StringTok{"ggplot2"}\NormalTok{)}
\KeywordTok{variable_importance}\NormalTok{(rf_explain) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{plot}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{single_variable}\NormalTok{(rf_explain, }\StringTok{"Age"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{plot}\NormalTok{() }\OperatorTok{+}\StringTok{ }\KeywordTok{ylim}\NormalTok{(}\FloatTok{0.2}\NormalTok{,}\FloatTok{0.8}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-49-2.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{single_variable}\NormalTok{(rf_explain, }\StringTok{"Pclass"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{plot}\NormalTok{() }\OperatorTok{+}\StringTok{ }\KeywordTok{ylim}\NormalTok{(}\FloatTok{0.2}\NormalTok{,}\FloatTok{0.8}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-49-3.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{single_variable}\NormalTok{(rf_explain, }\StringTok{"Fare"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{plot}\NormalTok{() }\OperatorTok{+}\StringTok{ }\KeywordTok{ylim}\NormalTok{(}\FloatTok{0.2}\NormalTok{,}\FloatTok{0.8}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-49-4.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{single_variable}\NormalTok{(rf_explain, }\StringTok{"Sex"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{plot}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-49-5.pdf}
...
\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-51-2.pdf}
...
##      -3.0241 (1829), -2.5484 (1180), -2.5117 (456), -2.5023 (267), -2.2768 (920) 
##     Top highest standardised residuals: 
##      15.378 (1914), 10.802 (1745), 10.496 (1532), 9.4129 (1111), 8.5633 (1051) 
##   -----------------------------------------------
##    Checks for trend in residuals
##   -----------------------------------------------
##     Model name:  ranger 
##     Standardised loess fit:  +56.09     ***
\end{verbatim}
...
\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-52-1.pdf}
...
\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-52-2.pdf}
...
\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-52-3.pdf}
...
\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-52-4.pdf}
...
\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-52-5.pdf}
