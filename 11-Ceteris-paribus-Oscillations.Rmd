...
(ref:CPVIPprofilesDesc) The value of the colored area summarizes the ceteris-paribus (CP) profile oscillations and provides the mean of the absolute deviations between the CP profile and the instance prediction (for Henry). Panel A shows plots for continuous explanatory variables, while panel B shows plots for categorical variables in the `titanic_rf` random-forest model for the Titanic data. [TOMASZ: STATISC FIGURE, CHECK FOR CORRECTNESS.]
...
Figure \@ref(fig:CPVIP1) provides a barplot of variable-importance measures for different continuous explanatory variables for the random-forest model `titanic_rf` (see Section \@ref(model-titanic-rf)) for Henry, a 47-year-old man who travelled in the first class  (see Section \@ref(predictions-titanic)). The measures were computed by using estimator $\widehat{vip}_{CP}^{j,uni}(\underline{x}_*)$, given in \@ref(eq:VIPCPuni), by using an equidistant grid of values for continuous variables.
...
(ref:CPVIP1Desc) Variable-importance measures based on ceteris-paribus oscillations estimated by using a uniform grid of explanatory-variable values for the `titanic_rf` model and passenger Henry for the Titanic data.
...
(ref:CPoscDefForHenryDesc) Variable-importance measures based on ceteris-paribus oscillations estimated by the default method of the `variable_attribution()` function for the `titanic_rf` model and Henry for the Titanic data.
...
oscillations_equidist <- variable_attribution(explain_rf, henry, 
              variable_splits = list(age = seq(0, 65, 0.1),
                                    fare = seq(0, 200, 0.1),
                                   sibsp = seq(0, 8, 0.1),
                                   parch = seq(0, 8, 0.1),
...
<!---
(ref:CPoscGridForHenryDesc) Variable-importance measures based on ceteris-paribus oscillations estimated by using a uniform grid of explanatory-variable values in the `variable_attribution()` function for the `titanic_rf` model and passenger Henry for the Titanic data.
--->
