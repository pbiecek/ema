...
In Chapter \@ref(ceterisParibusOscillations), we introduced a method to assess the importance of an explanatory variable for an instance prediction based on Ceteris-paribus (CP) profiles. The main limitation of the method is that the sum of the importance scores for all variables does not correspond to the model prediction.

In this chapter, we introduce Break-down (BD) plots which address this problem. The method is similar to the EXPLAIN algorithm introduced in [@explainPaper] and implemented in the `EXPLAIN` [@explainPackage] package.

## Intuition

We want to attribute a part of the model prediction to a single explanatory variable. To do so, we may consider calculating the contribution of a particular variable as a shift in the model prediction after conditioning over consecutive variables.

Assume a classical linear model for dependent variable $Y$ with $p$ explanatory variables. The prediction for $Y$ at point $X=x=(x_1, x_2, \ldots, x_p)$ is given by the expected value of $Y$ conditional on $X=x$. For a linear model, the expected value is given by the following linear combination:
$$ E_Y(Y | x) = f(x) = \beta_0 + x_1 \beta_1 + \ldots + x_p \beta_p. $$
We are interested in the contribution of the $k$-th explanatory variable to model prediction $f(x^*)$ for a single observation described by $x_*$. In this case, the contribution is equal to $x_*^k\beta_k$, because the $k$-th variable occurs only in this term. As it will become clear in the sequel, it is easier to interpret the variable’s contribution if $x^k$ is centered by subtracting a constant $\hat x^k$ (usually, the mean of $x^k$). This leads the following, intuitive formula for the variable attribution: $$ v(f, x_*, k) = \beta_k (x_*^k - \hat x^k).$$
