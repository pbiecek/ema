...
r_i = y_i - f(x_i).
$$
Note that, for a binary variable, the residual is the difference between 0 or 1, depending on how we code "success," and the predicted probability of "success." 
For multiresponse models it is common to perform such analysis for each class separately, in the one-vs-all settings.
[PBI: updated]  
...
As an example, we will use the predictions for the random forest model for the Titanic data (see Section \@ref(model-titanic-rf)). We show profiles only for a single continuous explanatory variable, *age*. This makes the example easier to read without reducing the generality
...
[PBI:added] Figure \@ref(fig:localFidelityPlots2) presents an example for categorical variable `class`. Henry and his neighbors traveled in class `1st` and in corresponding panels we see how particular odds of survival change according to model predictions if `1st` class is replaced by `2nd` (in most cases they will be lowered) or `deck crew` (in most cases they will increase). Such plots help to detect interactions, as we see that for different passengers the same change (let's say from `1st` class to `3rd` class) results in different model responses (for some passengers it increases for other decreases predictions).
...
```{r localFidelityPlots2, echo=FALSE, fig.cap="(fig:localFidelityPlots2) Elements of a local-fidelity plot for categorical variable. The plot summarizes categorical variable `class`. In each panel we present conditional model responses how model prediction would change if the class is changed from class `1st` to a selected class. The local fidelity problem is constructed for Henry and his 10 neighbors. Dots stands for original model predictions for these neighbors as if they are in class `1st`, end of the interval corresponds to model predictions after swapping class to a selected one. In the first panel we see that for most neighbors change from class `1st` to `2nd` decreases model response, maybe except two neighbors with low chances anyway. On the contrary third panel corresponds to the `deck crew` members, for most neighbors chances of survival increase after changing the class from `1st` to `desk crew`.", out.width = '70%', fig.align='center'}
