...
PBI there are 7, 14, 5 missing values in the 1st/2nd/3rd class. Fare differs among classes, so maybe it should be a class average?]
...
# tapply(titanic$fare, titanic$class, mean, na.rm = TRUE)
...
[TOMASZ:  CHANGE THE ORDER, AS SUGGESTED IN THE R CODE. LABEL THE HORIZONTAL "SURVIVAL" AXIS WITH PROPORTIONS. IMPROVE LABELING OF THE Y-AXIS. ]

Figures \@ref(fig:titanic_exploration_gender) and \@ref(fig:titanic_exploration_age)  indicate that the proportion of survivors was larger for females and children below 5 years of age. This is most likely the result of the "women and children first" principle that is often evoked in situations that require evacuation of persons whose life is in danger. The principle can, perhaps, partially explain the trend seen in Figures \@ref(fig:titanic_exploration_parch) and \@ref(fig:titanic_exploration_sibsp), i.e., a higher proportion of survivors among those with 1-3 parents/children and 1-2 siblings/spouses aboard. Figure \@ref(fig:titanic_exploration_class)  indicates that passengers travelling in the first and second class had a higher chance of survival, perhaps due to the proximity of the location of their cabins to the deck. Interestingly, the proportion of survivors among crew deck was similar to the proportion of the first-class passengers. Figure \@ref(fig:titanic_exploration_fare)  shows that the proportion of survivors increased with the fare, which is consistent with the fact that the proportion was higher for passengers travelling in the first and second class. Finally, Figures \@ref(fig:titanic_exploration_embarked) and \ref(fig:titanic_exploration_country) do not suggest any noteworthy trends.        

```{r titanic_exploration_gender, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:titanic_exploration_gender) Survival in different genders for the titanic data.", out.width = '70%', fig.align='center'}
library("ggmosaic")
ggplot(data = titanic) +
...
```{r titanic_exploration_age, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:titanic_exploration_age) Survival in different age groups for the titanic data.", out.width = '70%', fig.align='center'}
...
```{r titanic_exploration_parch, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:titanic_exploration_parch) Survival in different numbers of parents/children for the titanic data.", out.width = '70%', fig.align='center'}
ggplot(data = titanic) +
...
```{r titanic_exploration_sibsp, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:titanic_exploration_sibsp) Survival in different numbers of siblings/spouses for the titanic data.", out.width = '70%', fig.align='center'}
ggplot(data = titanic) +
...
```{r titanic_exploration_class, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:titanic_exploration_class) Survival for different classes in the titanic data.", out.width = '70%', fig.align='center'}
ggplot(data = titanic) +
...
```{r titanic_exploration_fare, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:titanic_exploration_fare) Survival as a function of fare in the titanic data.", fig.width=6, fig.height=2, echo=FALSE}
library("ggplot2")
...
```{r titanic_exploration_embarked, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:titanic_exploration_embarked) Survival for different port of embarking in the titanic data.",fig.width=6, fig.height=4}
library("ggmosaic")
ggplot(data = titanic) +
...
```{r titanic_exploration_country, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:titanic_exploration_country) Survival for different countries in the titanic data.",fig.width=6, fig.height=4}
ggplot(data = titanic) +
...
Note that this is artificial dataset created to explain some differences between random forest and linear regression. So the data is more clean than a usual dataset. Nerveless let do some exploration.

Distribution of square meter is slightly skewed. 

```{r appartments_exploration_mi2, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:appartments_exploration_mi2) Survival for different countries in the titanic data."}
ggplot(data = apartments) +
   geom_histogram(aes(m2.price), binwidth = 100, color = "white") +
   labs(x="Price per square meter", title='Distribution') + theme_drwhy() + theme(legend.position = "none") 
```

There is some nonlinear (if any) relation between construction year and price.

```{r appartments_mi2_construction, warning=FALSE, message=FALSE, echo=FALSE, fig.width=4, fig.height=4, fig.cap="(fig:appartments_mi2_construction) Construction year vs price per sq meter"}
ggplot(data = apartments, aes(construction.year, m2.price)) +
   geom_point(size = 0.3) +
  geom_smooth(se = FALSE, size=1, color = "#371ea3") +
   labs(y="Price per square meter", x = "Construction year", title='Relation between square m2 and contruction year') + theme_drwhy() + theme(legend.position = "none") 
```

There is a sharp linear relation between surface and price.

```{r appartments_mi2_surface, warning=FALSE, message=FALSE, echo=FALSE, fig.width=4, fig.height=4, fig.cap="(fig:appartments_mi2_surface) Surface vs price per sq meter"}
ggplot(data = apartments, aes(surface, m2.price)) +
   geom_point(size = 0.3) +
  geom_smooth(se = FALSE, size=1, color = "#371ea3") +
   labs(y="Price per square meter", x = "Surface year", title='Relation between surface and contruction year') + theme_drwhy() + theme(legend.position = "none") 
```

There is a close to linear relation between floor and price. 

```{r appartments_mi2_floor, warning=FALSE, message=FALSE, echo=FALSE, fig.width=4, fig.height=4, fig.cap="(fig:appartments_mi2_floor) Floor vs price per sq meter"}
ggplot(data = apartments, aes(floor, m2.price)) +
   geom_point(size = 0.3) +
  geom_smooth(se = FALSE, size=1, color = "#371ea3") +
   labs(y="Price per square meter", x = "Floor", title='Relation between floor and price per square meter') + theme_drwhy() + theme(legend.position = "none") 
```

There is a close to linear relation between no of rooms and price. But please note that surface and number of rooms are correlated.

```{r appartments_mi2_norooms, warning=FALSE, message=FALSE, echo=FALSE, fig.width=4, fig.height=4, fig.cap="(fig:appartments_mi2_norooms) No rooms vs price per sq meter"}
ggplot(data = apartments, aes(no.rooms, m2.price)) +
   geom_point(size = 0.3) +
  geom_smooth(se = FALSE, size=1, color = "#371ea3") +
   labs(y="Price per square meter", x = "No rooms", title='Relation between no rooms and price per square meter') + theme_drwhy() + theme(legend.position = "none") 
```

```{r appartments_surface_norooms, warning=FALSE, message=FALSE, echo=FALSE, fig.width=4, fig.height=4, fig.cap="(fig:appartments_surface_norooms) No rooms vs surface"}
ggplot(data = apartments, aes(no.rooms, surface)) +
   geom_point(size = 0.3) +
  geom_smooth(se = FALSE, size=1, color = "#371ea3") +
   labs(y="Surface", x = "No rooms", title='Relation between no rooms and price per square meter') + theme_drwhy() + theme(legend.position = "none") 
```

Prices are different in different districts. Highest are in the Downtown.

```{r appartments_mi2_district, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:appartments_mi2_district) District vs price per sq meter"}
...
ggplot(data = apartments, aes(district, m2.price)) +
  ...
  geom_boxplot(width = 0.2, coef = 100, color = "white", fill = "#371ea3") +
   labs(y="Price per square meter", x = "", title='Relation between district and price per square meter') + theme_drwhy() + theme(legend.position = "none") + coord_flip()
```
...
* *evaluation*, the last evaluation score, a numerical variable with values 2 (fail), 3 (satisfactory), 4 (good), and 5 (veru good) 
...
Young female and old male are being fired more frequently.

```{r HR_exploration_age, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:HR_exploration_age) HR status for age groups and gender.", out.width = '70%', fig.align='center'}
...
ggplot(data = HR) +
   ...
   labs(x="Gender and age", y="Promotions vs gender and age", title='Promoted of fired?') + theme_drwhy() + theme(legend.position = "none") + coord_flip() + scale_fill_manual(values = theme_drwhy_colors(3))
```

```{r HR_exploration_salary, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:HR_exploration_salary) HR status for salary groups.", out.width = '70%', fig.align='center'}
ggplot(data = HR) +
   geom_mosaic(aes(x = product(status, salary), fill=status)) +
   labs(x="Salary leve", y="Promoted of fired?", title='Promotions vs salary') + theme_drwhy() + theme(legend.position = "none") + coord_flip() + scale_fill_manual(values = theme_drwhy_colors(3))
```

```{r HR_exploration_evaluation, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="(fig:HR_exploration_evaluation) HR status for evaluation groups.", out.width = '70%', fig.align='center'}
ggplot(data = HR) +
   geom_mosaic(aes(x = product(status, evaluation), fill=status)) +
   labs(x="Evaluation", y="Promoted of fired?", title='Promotions vs evaluation') + theme_drwhy() + theme(legend.position = "none") + coord_flip() + scale_fill_manual(values = theme_drwhy_colors(3))
```
