...
Explainers listed in the previouse point were focused on a question, how variables are linked with the model prediction. 
Here we will present methods, that are linked with a questions, how changes in variables affect model predictions. Instead of being focused on a response for a single input, we would like to explore model responses around the point of interest.

Such explainers are linked with the second right listed in [ref:PredictionExplainers]. Such speculations are designed to present / visualise model response for input that is sighty changed.


Think of following usecases:

A. For a patient Alicia the model retuns high risk of a hypertension. The patient would like to know how to reduce this risk. Here the patient would like to know, how his risk of hupertension would change if a particular variable will change (like smoking status, physical activity, diet).
B. For customer Bob the credit scoring model retunrs high risk of default therefore bank refuses the credit. Here Bob would like to know what needs to be changed to pass the bank criteria. To change the model decision 
C. 
