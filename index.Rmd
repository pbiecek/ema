...
It is worth noting that, When it comes to predictive models, the same concepts have often been given different names in statistics and in machine learning. For instance, in the statistical-modelling literature, one refers to ,,explanatory variables,'' with ,,independent variables,'' ,,predictors,'' or ,,covariates'' as often-used equivalents. The explanatory variables are used in the model as means to explain (predict) the ,,dependent variable,'' also called the ,,response'' or ,,predicted'' variable. In the machine-learning language, ,,input variables'' or ,,features'' are used to predict the ,,output'' variable. In statistical modelling, models are fit to the data that contain ,,observations,'' whereas in the machine-learning word a dataset may contain ,,instances.'' To the extent possible, in our book we will try to consistently use the statistical terminology. However, the reader may expect an accidental reference to a feature here and there.   
...
We will often refer to ,,black-box'' models. These are models with a complex structure that is hard to understand by  humans. Usually it refers to the number of model coefficients or parameters. As different humans may be better in understanding more or less complex models, there is no strict threshold that makes model a black-box. In practice, for most humans this threshold is probably closer to 10 rather than 100. 

A ,,white-box'' model, which is  opposite to a ,,black-box'' one, is a model that is easy to understand by a human (though maybe not by every human). Ann example is a linear model with a few coefficeitns or a small classification-and-regression tree.
