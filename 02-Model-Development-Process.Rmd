...
For statistical models it is common to assume some family of probability distributions for $y|x$. In such case the loss function $L$ may be defined as a minus log-likelihood function for $\theta$. Likelihood is probability of observing $x$ as a function of parameter $\theta$.
...
For linear regression, the penalty term $\lambda(\beta)$ is equal to $0$, and optimal parameters $\beta$ in equation \@ref(eq:modelTrainingEq2) have close analytical solution. In ridge regression the penalty  $\lambda(\beta) = ||\beta||_2$ and also  \@ref(eq:modelTrainingEq2) have analytical solution. For LASSO regression the penalty  $\lambda(\beta) = ||\beta||_1$ and $\beta$ are estimated through a numerical optimization.
