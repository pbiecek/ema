...
For classical linear model, the proposed variable-attribution approach offers the following advantages:
- The defintion of $v(f, x^*, i)$ implies that it is not an approximation and it is directly linked with the structure of a model.
- The attibution is linked neither with scale nor location of $X_i$; hence, it is easier to understand than, for instance, the standardized value of $\beta_i$. [TOMASZ: I AM NOT SURE ABOUT THIS ARGUMENT. THE DEFINITION OF v() INVOLVES CENTERING.]

An obvious diasadvantage is that the definition of $v(f, x^*, i)$ is very much linear-model based. Also, does not, in any way, reduce the model complexity; it only presents model coefficients in a different way. [TOMASZ: I AM NOT SURE ABOUT THIS ARGUMENT. MODEL COMPLEXITY IS WHAT IT IS. HOW CAN WE RUDCE IT?]


## Code snippets

In this section, we will present an example of computing variable attributions using the `HR` dataset (see Section \@ref(HRdataset) for more details). 
...
Variable attributions for linear and generalized linear models may be directly extracted by applying the `predict()` function, with the argument `type = "terms"`, to an object containing results of fitting of a mode. To illustrate the approach for a logistic regression, we build a logistic regression model for binary variable `status == "fired"` and extract the estimated model coefficients:
...
Below we illustrate how to do this with the `DALEX` package. The use of the package allows for an easy construction of a plot presenting the variable attributions.
