...
In this chapter we introduce Break-down (BD) plots, which offer a solution to this problem. Note that the method is similar to the `EXPLAIN` algorithm introduced in  [@explainPaper] and implemented in the `ExplainPrediction` package [@explainPackage].
...
The `iBreakDown::break_down()` function calculates the varaible-importance measures for a selected model and the instance of interest. The result of applying the `break_down()` function is a data frame containg the calculated measures. In the simplest call, the function requires only two arguments: the model explainers and the data frame for  the instance of interest. The call below essentailly re-creates the variable-importance values ($\Delta^{j|\{1,\ldots,j\}}$) presented in Table \@ref(tab:titanicBreakDownDeltasConseq).
