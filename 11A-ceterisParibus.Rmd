...
The true distribution of $j$-th explanatory variable is, in most cases, unknown. Thus we have few options how to calculate \@ref(eq:VIPCPdef). 

One is to calculate just the area under the Ceteris Paribus curve, what corresponds to the uniform distribution for $g^j(z)$ in the range of variable $x^j$. A straightforward estimator of $vip_{CP}^{j,uni}(x_*)$ is

\begin{equation}
\widehat{vip}_{CP}^{j,uni}(x_*) = \frac 1k \sum_{i=1}^k |h^{j}_{x_*}(x^{j}_k) - f(x_*)|,
(\#eq:VIPCPest)
\end{equation}

Where $x^{j}_k$ is a vector of $k$ points selected for variable $x^{j}$. For a continuous variable one can use equispaced points, or for any variable one can use all unique values for $x^{j}$ in the considered dataset.

Another approach is to use empirical marginal distribution for $x^{j}$. This leads to a straightforward estimator of $vip_{CP}^{j,emp}(x_*)$ defined as
...
**Note** The choice of $\widehat{vip}_{CP}^{j,emp}(x_*)$ is prefered when we have enough data to accurately estimate the empirical distribution and then the distribution is not uniform. The $\widehat{vip}_{CP}^{j,uni}(x_*)$ is in most cases quicker to calculate and is preferred if we look for fast approximations. 



**Note** The importance of an explanatory variable for instance prediction may be very different for different points $x_*$. For example, consider model 
$$
f(x_1, x_2) = x_1 * x_2,
$$
...
Figure \@ref(fig:CPVIP1) provides a barplot of variable importance measures for different continuous explanatory variables for the random forest model `titanic_rf_v6` for `henry`. [PBI: changed name]
...
If Henry were older, this would significantly lower the chance of survival. One the other hand, were Henry not travelling alone, this would increase the chance. 
...
Oscillations of CP profiles are easy to interpret and understand. By using the average of oscillations, it is possible to select the most important variables for an instance prediction. The methodology can easily be extended to two or more variables. 

There are several issues related to the use of the CP oscillations. For example, the oscillations may not be of help in situations when the use of CP profiles may itself be problematic (e.g., in the case of correlated explanatory variables or interactions - see Section \@ref(CPProsCons)). An important issue is that the local variable importances do not sum up to the instance prediction for which they are calculated. In Chapters \@ref(breakDown) and \@ref(shapley), we will introduce measures that address this problem.
...
The resulting object can subsequently be processed with the `calculate_oscillations()` function to calculate the oscillations and the variable-importance-measure estimates \@ref(eq:VIPCPest).
...
Note, that the `calculate_oscillations()` function calculates $\widehat{vip}_{CP}^j(x_*)$, as given in \@ref(eq:VIPCPest), as an average absolute difference between the CP profile and the model prediction for a selected instance over a grid of values.

The choice of grid of values may influence the result, as it defines the distribution over which the average \@ref(eq:VIPCPest), corresponding to \@ref(eq:VIPCPdef), will be computed.

The default choice, implemented in the `calculate_oscillations()` function, is to use all unique values of the explanatory variable as the grid points which correspond to the estimator $vip_{CP}^{j,uni}(x_*)$ as introduced in \@ref(eq:VIPCPdef).
[PBI: updated] However, other approaches are also possible.

One is to calculate the expectation \@ref(eq:VIPCPdef) assuming a uniform distribution for all explanatory variables. Toward this aim, we have got to explicitly specify a dense uniform grid of values for each explanatory variable used in the model. The `variable_splits` argument can be used for this purpose.
...
Another approach is to calculate the expectation \@ref(eq:VIPCPdef) over the empirical distribution of each variable as defined in formula for $vip_{CP}^{j,emp}(x_*)$. 

Toward this aim, we use the `variable_splits` argument to explicitly specify the validation-data sample to define the  grid of values. 
