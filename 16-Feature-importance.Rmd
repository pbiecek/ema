...
```{r TitanicRFFeatImp, warning=FALSE, message=FALSE, echo=FALSE, fig.width=5, fig.height=2.5, fig.cap='(ref:TitanicRFFeatImpDesc)', out.width = '70%', fig.align='center'}
library("ingredients")
library("ggplot2")
set.seed(1259)
fi_rf <- model_parts(explain_titanic_rf, loss_function = loss_one_minus_auc, B = 1, N = NULL) 
plot(fi_rf[-2,], show_boxplots = FALSE, bar_width = 5) + 
...
```{r TitanicRFFeatImp10, warning=FALSE, message=FALSE, echo=FALSE, fig.width=5, fig.height=2.5, fig.cap='(ref:TitanicRFFeatImp10Desc)', out.width = '70%', fig.align='center'}
set.seed(1259)
fi_rf <- model_parts(explain_titanic_rf, loss_function = loss_one_minus_auc, B = 10, N = NULL) 
plot(fi_rf[-2,], bar_width = 5) + 
  ggtitle("Permutation-based variable-importance","") + 
  ylab("Loss function: 1-AUC") + ylim(0.1,0.35)
```
...
```{r TitanicFeatImp, warning=FALSE, message=FALSE, echo=FALSE, fig.width=5, fig.height=6, fig.cap='(ref:TitanicFeatImpDesc)', out.width = '70%', fig.align='center'}
library("gbm")
library("rms")
titanic_lmr_v6 <- archivist::aread("pbiecek/models/58b24")
explain_titanic_lmr <- DALEX::explain(model = titanic_lmr_v6, data = titanic_imputed[, -9],
...
explain_titanic_gbm <- DALEX::explain(model = titanic_gbm_v6, data = titanic[, -9],
...
```{r featureImportanceUnoPlot, fig.cap='(ref:featureImportanceUnoPlotDesc)', warning=FALSE, message=FALSE, fig.width=6, fig.height=2.5, fig.align='center', out.width = '70%'}
library("ggplot2")
plot(vip.50) +
...
```{r featureImportanceTriPlot, fig.cap='(ref:featureImportanceTriPlotDesc)',  warning=FALSE, message=FALSE, fig.width=6, fig.height=5, fig.align='center', out.width = '70%'}
library("ggplot2")
...
```{r examplePythonFIM1, echo=FALSE, out.width = '100%', fig.align='center'}
...
```{r examplePythonFIM2, echo=FALSE, out.width = '100%', fig.align='center'}
...
For example, if we want to analyse importance of groups of variables, we can set these groups in a following manner.

```{python,  eval = FALSE}
vi_grouped = exp_rf.model_parts(variable_groups={'personal': ['gender', 'age', 'sibsp', 'parch'],
                                     'wealth': ['class', 'fare']})
vi_grouped.result
```
