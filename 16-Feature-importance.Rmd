...
```{r TitanicRFFeatImp, warning=FALSE, message=FALSE, echo=FALSE, fig.width=5, fig.height=2.5, fig.cap="Each interval presents the difference between the loss function for the original data (vertical dashed line at the left) and for the data with permuted observation for a particular variable."}
library("ingredients")
fi_rf <- feature_importance(explain_titanic_rf, loss_function = loss_one_minus_auc, B = 1) 
plot(fi_rf[-2,], show_boxplots = FALSE) + 
  ggtitle("Permutatinal variableimportance") + 
...
library("randomForest")
set.seed(59)
explainer_rf <- archivist::aread("pbiecek/models/f49ea")
```
