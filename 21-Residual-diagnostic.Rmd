...
Let us consider the classical linear-regression model. In that case, residuals should be normally distributed with mean zero and variance defined by the diagonal of hat-matrix $\underline X(\underline X^T \underline X)^{-1}\underline X^T$. For independent explanatory variables it leads to a constant variance of residuals. Figure \@ref(fig:residuals1234) presents examples of classical diagnostic plots for linear models that can be used to check whether the assumptions are fulfilled. In fact, the plots in Figure \@ref(fig:residuals1234) suggest issues with the assumptions. 
...
(ref:residuals1234Desc) Diagnostic plots for a linear-regression model. Clockwise from the top-left: residuals as a function of fitted values, a scale-location plot, a nomal quantile-quantile plot, and a leverage plot. In each panel, the three most extreme observations have indexes shown in the data set.
...
Despite the similar value of RMSE, the distributions of residuals for both models are different. In particular, Figure \@ref(fig:plotResidualDensity1) indicates that the distribution for the linear-regression model is, in fact, split into two separate, normal-like parts, which may suggest omission of a binary explanatory variable in the model. The two components are located around the values of about -200 and 400. As mentioned in the previous chapters, the reason for this behavior of the residuals is the fact that the model does not capture the non-linear relationship between the price and the year of construction. As we read from Figure \@ref(fig:pdpApartment3) the relationship between the year of production and the price looks like a stepped function. Apartments built between 1940 and 1990 are on average cheaper than those built earlier or later.
...
plot(md_rf, variable = "y", yvariable = "residuals") + xlab("true value of m2.price (y)") + ylab("residuals (r_i)")
```
...
plot(md_rf, variable = "y_hat", yvariable = "residuals") + ylab("residuals") + xlab("predicted m2.price (y_hat)")
```
...
For classical models, like linear regression, such heterogeneity of the residuals would be worrying. But in case of a random forest it is caused by the nature of the model. A random forest reduces variation of residuals by introducing a bias, in this case bias towards the average. It is the decision of model creator to choose whether such a bias for the most expensive and cheapest apartments is a desired property for the apartments pricing
...
```{r plotScaleLocation1, fig.cap='(ref:plotScaleLocation1Desc)',  warning=FALSE, message=FALSE, echo = FALSE, fig.width=5, fig.height=5,  fig.align='center'}
plot(md_rf, variable = "y_hat", yvariable = "abs_residuals") + 
    xlab("predicted m2.price") + ylab("absolute value of residuals")
```
