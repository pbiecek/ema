...
The subset of 5000 most valuable players from FIFA 19 data is available in `fifa` data frame in the `DALEX` package.
...
The code below fits the models to the data. The dependent variable `LogValue` contains the base-10 logarithm of `Value.EUR`, i.e., the player's value.
...
The code below fits the models to the data. The dependent variable `ylog` contains the logarithm of `value_eur`, i.e., the player's value.
...
Subsequently, we create a plot illustrating the computed values. 
...
In Chapter \@ref(ceterisParibus), we introduced ceteris-paribus (CP) profiles. They capture the effect of a selected explanatory variable in terms of changes in a model's prediction induced by changes in the variable's values. 
...
As it was argued in Chapter \@ref(localDiagnostics), it is worthwhile to check how the model behaves for observations similar to the instance of interest. Towards this aim, we may want to compare the distribution of residuals for "neighbours" of Robert Lewandowski. <!-- Function `individual_diagnostics()` from the `DALEX` package provides the necessary functionality. First, we use it to select 30 closest neighbours and conduct the necessary computations for them. -->
...
Subsequently, we construct a plot that shows the histogram of residuals for all data and the 30 neighbours.

(ref:usecaseFIFAceterisParibusNeighboursPlotDesc) Distribution of residuals for all players and neighbours of Robert Lewandowski.
...
The resulting plot is presented in Figure \@ref(fig:usecaseFIFAceterisParibusNeighboursPlot). Clearly, the neighbours of Robert Lewandowski include some of the most expensive players. Therefore, as compared to the overall distribution, the distribution of residuals for the neighbours is skewed to the right and their mean is larger as the overall mean.  Thus, the model underestimates the actual value of the most expensive players. This was also noted based on the plot in the bottom-left panel of Figure \@ref(fig:modelPerformanceScatterplot).

We can also look at the local-stability plot, i.e., the plot that includes CP profiles for the nearest neighbours and the corresponding residuals (see Chapter \@ref(localDiagnostics)). We will limit ourselves to the plot for `Age`. 
