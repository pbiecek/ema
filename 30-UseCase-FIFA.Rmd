...
The subset of 5,000 most valuable players from the FIFA 19 data is available in the `fifa` data frame in the `DALEX` package.
...
The subset of 5,000 most valuable players from FIFA 19 data can be loaded to Python with `dalex.datasets.load_fifa()` method.
...
(ref:modelPerformanceScatterplotDesc) Observed and predicted (fitted) player's values for the four models for the FIFA 19 data.
...
The `model_diagnostics()` function (see Section \@ref(RcodeResidualDiagnostic)) is used to create residual-diagnostics plots. Results of this function can be visualised with the generic `plot()` function. In the code below, additional arguments are used to improve the outlook and iterpretability of both axes.
...
(ref:featureImportancePlotDesc) Mean variable-importance calculated using 10 permutations for the four models for the FIFA 19 data. [TOMASZ: I DO NOT GET THE PLOT OF ALL FOUR MODELS, ONLY FOR GBM SHALLOW.]
...
Finally, the `predict_diagnostics()` function (see Section \@ref(cPLocDiagR)) allows calculating local-stability  plots. The generic `plot()` function can be used to plot these profiles for selected variables. [TOMASZ: NO YNTAX FOOR THE PLOT.]
...
```{python, eval=FALSE}
fifa_mp_gbm <- fifa_gbm.predict_profile()
