...
The Fédération Internationale de Football Association (FIFA) is a governing body of football (sometimes, especially in the USA, called soccer). FIFA ratings of football players can be found at `https://sofifa.com/`. Data from this website for 2019 were scrapped and made available at the Kaggle webpage `https://www.kaggle.com/karangadiya/fifa19`.
...
The dataset contains 89 variables that describe 16,924 players. The variables include information such as age, nationality, club, wage, etc. In what follows, we focus on 45 variables that are included in data frame `fifa19small`. The variables are listed below.  
...
```{r usecaseFIFAshap, warning=FALSE, message=FALSE, echo=TRUE}
set.seed(1990)
...
```{r usecaseFIFAshapPlot, warning=FALSE, message=FALSE, echo=TRUE, fig.width=7, fig.height=5.5, fig.cap='(ref:usecaseFIFAshapPlotDesc)', out.width = '60%', fig.align='center'}
...
The profiles can be obtained with the help of function `individual_profile()` in the `DALEX` package (see Section \@ref(CPR)). We first have got to compute and store the values of the profiles for the selected variables. In the code below we use argument `variable_splits` to provide the values, at which the profiles are to be computed. 
<!---
...
id_rf_v6 <- individual_diagnostics(fifa_rf_exp, fifa19small_red["R. Lewandowski",],
                          neighbors = 30)
```

Subsequently, we construct a plot that shows the histrogram of residuals for all data and the 30 neighbors.
...
id_rf_v6_age <- individual_diagnostics(fifa_rf_exp, fifa19small_red["R. Lewandowski",],
...
As a second example, we present explanations for the random-forest-model prediction for *Cristiano Ronaldo* (CR7). Below we list his characteristics, as included in the analyzed dataset. Note that Cristiano Ronaldo, as Robert Lewandowski, is also a striker. It might be thus of interest to compare the characteristics contributing to the model's predictions for the two players.
