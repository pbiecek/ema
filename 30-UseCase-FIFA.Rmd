...
The subset of 5000 most valuable players from FIFA 19 data is avaliable in `fifa` data frame in the `DALEX` package.
...
Figure \@ref(fig:distFIFA19scatter2) presents the scatter-plot matrix for the four selected variables. It indicates that all variables are positively correlated, though with a different strength. In particular, `BallControl` and `Dribbling` are strongly correlated, with the estimated correlation coefficient larger than 0.9. `Reactions` is moderately correlated with the other three variables. Finally, there is a moderate correlation between `Age` and `Reactions`, but not much correlation with `BallControl` and `Dribbling`.   
...
In this chapter we will show R snippets how the gradient boosting model was built. Other models were built in a similar way.
...
In this chapter we will show Python snippets how the gradient boosting model was built. Other models were built in a similar way.

The code below fits the models to the data. The dependent variable `ylog` contains the logarithm of `value_eur`, i.e., player's value.

```{python, eval=FALSE}
from lightgbm import LGBMRegressor

X = fifa.drop(["nationality", "overall", "potential", "value_eur", "wage_eur"], axis = 1)
y = fifa['value_eur']
ylog = np.log(y)
...
`model_performance()` (see Section \@ref(modelPerformanceR)), as indicated below.
...
In this chapter we will show R snippets for model audit for the gradient boosting model. Other models were audited in a similar way.

The `model_performance()` function is used to calculate RMSE, MSE, MAD and other performance measures for considered models.

```{r modelAuditFifaR, warning=FALSE, message=FALSE, echo=TRUE, eval=FALSE}
model_performance(fifa_gbm_exp_deep)
```

The `model_diagnostics()` function (see Section \@ref(RcodeResidualDiagnostic)) allows creation of various residual-based plots.
Results of this function can be visualised with the generic `plot()` function. In the code below additional beautifier are used for both axes.
...
All four developed models involve many explanatory variables. It is of interest to understand which of them exercises  the largest influence of models' predictions. Toward this aim, we can apply the permutation-based variable-importance measure discussed in Chapter \@ref(featureImportance). 
...
Subsequently, we can construct a plot of the obtained mean (over the default 10 permutations) variable-importance measures. Note that we consider only the top 20.   
...
plot(fifa_mp_gbm_shallow, fifa_mp_gbm_deep, fifa_mp_rf, fifa_mp_rm,
     max_vars = 20, bar_width = 4, show_boxplots = FALSE) + ggtitle("Feature Importance", "")
```
...
In this chapter we will show R snippets for model-level exploration for the gradient boosting model. Other models were audited in a similar way.

The `model_parts()` function from the `DALEX`package (see Section \@ref(featureImportanceR)) is used to calculate feature importance via feature permutations. The generic `plot()` function visualizes these statistics. The `max_vars` argument limit number of presented variable up to 20.

```{r modelLeveleModelsEx1, warning=FALSE, message=FALSE, eval=FALSE}
fifa_mp_gbm_deep <- model_parts(fifa_gbm_exp_deep)
plot(fifa_mp_gbm_deep, max_vars = 20, bar_width = 4, show_boxplots = FALSE)
```

The `model_profile()` function from the `DALEX`package (see Section \@ref(PDPR)) is used to calculate partial dependence profiles. The generic `plot()` function visualizes these profiles for selected variables.
...
In this chapter we will show Python snippets for model-level exploration for the gradient boosting model. Other models were audited in a similar way.

The `model_parts` method from the `dalex` library (see Section \@ref(featureImportancePython)) is used to calculate feature importance via feature permutations. The `plot()` method visualizes these statistics. 

```{python, eval=FALSE}
fifa_mp_gbm = fifa_gbm.model_parts()
fifa_mp_gbm.plot(max_vars=20)
```

The `model_profile` method from the `dalex` library (see Section \@ref(PDPPython)) is used to calculate partial dependence profiles. The `plot()` method visualizes these profiles for selected variables.

```{python, eval=FALSE}
fifa_mp_gbm = fifa_gbm.model_profile()

fifa_mp_gbm.plot(variables = ['Reactions', 'BallControl',
                               'Dribbling', 'Age'])
```
...
As a first example, we take a look at the value of *Robert Lewandowski*, for an obvious reason. Below we list his characteristics, as included in the analyzed dataset. Robert Lewandowski is a striker.
...
```{r usecaseFIFAbreakDownPlot, warning=FALSE, message=FALSE, echo=FALSE, fig.width=10, fig.height=4, fig.cap='(ref:usecaseFIFAbreakDowDesc)', out.width = '100%', fig.align='center'}
pl1 <- plot(fifa_bd_rf) +
...
```{r usecaseFIFAshapPlot, warning=FALSE, message=FALSE, echo=FALSE, fig.width=10, fig.height=4, fig.cap='(ref:usecaseFIFAshapPlotDesc)', out.width = '100%', fig.align='center'}
...
In Chapter \@ref(ceterisParibus), we introduced ceteris-paribus (CP) profiles. They capture the effect of a selected explanatory variable in terms of changes of a model's prediction induced by changes in the variable's values. 
...
plot(fifa_cp_rf, color = "_label_", 
  ...
  scale_y_continuous("Estimated value in Euro", trans = "log10", 
                     labels = dollar_format(suffix = "€", prefix = "")) +
  theme(legend.position = "none") + ggtitle("Ceteris Paribus profile", "")
```
...
Subsequently, we construct a plot that shows the histogram of residuals for all data and the 30 neighbors.

(ref:usecaseFIFAceterisParibusNeighboursPlotDesc) Distribution of residuals for all players and neighbors of Robert Lewandowski.

```{r usecaseFIFAceterisParibusNeighboursPlot, warning=FALSE, message=FALSE, echo=FALSE, fig.width=10, fig.height=4.5, fig.cap='(ref:usecaseFIFAceterisParibusNeighboursPlotDesc)', out.width = '90%', fig.align='center'}
plot(id_rf) 
```
...
We can also look at the local-stability plot, i.e., the plot that includes CP profiles for the nearest neighbors and the corresponding residuals (see Chapter \@ref(localDiagnostics)). We will limit ourselves to the plot for `Age`. 
...
```{r usecaseFIFAceterisParibusNeighboursAgeRFPlot, warning=FALSE, message=FALSE, echo=FALSE, fig.width=10, fig.height=5, fig.cap='(ref:usecaseFIFAceterisParibusNeighboursAgeRFPlotDesc)', out.width = '90%', fig.align='center'}
plot(id_rf_age) +  
  scale_y_continuous("Estimated value in Euro", trans = "log10", 
                     labels = dollar_format(suffix = "€", prefix = "")) +   
...
In this chapter we will show R snippets for instance-level exploration for the gradient boosting model. Other models were audited in a similar way.

The `predict_parts()` function from the `DALEX`package (see Chapters \@ref(breakDown)-\@ref(shapley)) is used to calculate variable attribution. The generic `plot()` function visualizes these attribution. 

Use `type = "break_down"` to calculate break-down plots.

```{r instanceLeveleModelsEx1, warning=FALSE, message=FALSE, eval=FALSE}
fifa_bd_gbm <- predict_parts(fifa_gbm_exp, new_observation = fifa["R. Lewandowski",], type = "break_down")
...
Or use `type = "shap"` to calculate Shapley values.

```{r instanceLeveleModelsEx2, warning=FALSE, message=FALSE, eval=FALSE}
fifa_shap_gbm <- variable_attribution(fifa_gbm_exp, new_observation = fifa["R. Lewandowski",], type = "shap")
...
The `predict_profile()` function from the `DALEX` package (see Section \@ref(CPR)) is used to calculate cateris paribus profiles. The generic `plot()` function visualizes these profiles for selected variables.
...
Finally `predict_diagnostics()` function (see Section \@ref(cPLocDiagR)) is used to calculate local diagnostic plots. The generic `plot()` function visualizes these profiles for selected variables.

```{r modelLeveleModelsEx4, warning=FALSE, message=FALSE, eval=FALSE}
id_rf <- predict_diagnostics(fifa_rf_exp, 
                         fifa["R. Lewandowski",],
                         neighbors = 30)
```
...
In this chapter we will show Python snippets for instance-level exploration for the gradient boosting model. Other models were audited in a similar way.

The `predict_parts` method from the `dalex` library (see Sections \@ref(BDPython), \@ref(SHAPPythonCode)) is used to calculate feature importance via feature permutations. The `plot()` method visualizes these statistics. 

```{python, eval=FALSE}
fifa_mp_gbm = fifa_gbm.predict_parts()
fifa_mp_gbm.plot(max_vars = 20)
```

The `predict_profile` method from the `dalex` library (see Section \@ref(PDPPython)) is used to calculate ceteris paribus profiles. The `plot()` method visualizes these profiles for selected variables.

```{python, eval=FALSE}
fifa_mp_gbm <- fifa_gbm.predict_profile(type = 'break_down')
...
As a second example, we present explanations for the random-forest-model's prediction for *Cristiano Ronaldo* (CR7). Below we list his characteristics, as included in the analyzed dataset. Note that Cristiano Ronaldo, as Robert Lewandowski, is also a striker. It might be thus of interest to compare the characteristics contributing to the model's predictions for the two players.
...
```{r usecaseFIFAbreakDownCR7Plot, warning=FALSE, message=FALSE, echo=FALSE, fig.width=10, fig.height=5, fig.cap='(ref:usecaseFIFAbreakDownCR7PlotDesc)', out.width = '100%', fig.align='center'}
...
```{r usecaseFIFAshapCR7Plot, warning=FALSE, message=FALSE, echo=FALSE, fig.width=10, fig.height=5, fig.cap='(ref:usecaseFIFAshapCR7PlotDesc)', out.width = '100%', fig.align='center'}
set.seed(1965)
...
One might be interested in the characteristics influencing the random-forest model's predictions for players other than strikers. To address the question, we present explanations for *Wojciech Szczęsny*, a goalkeeper. Below we list his characteristics, as included in the analyzed dataset. 
...
```{r usecaseFIFAbreakDownWS, warning=FALSE, message=FALSE, echo=FALSE, fig.width=10, fig.height=5, fig.cap='(ref:usecaseFIFAbreakDownWSDesc)', out.width = '100%', fig.align='center'}
...
```{r usecaseFIFAshapWS, warning=FALSE, message=FALSE, echo=FALSE, fig.width=10, fig.height=5, fig.cap='(ref:usecaseFIFAshapWSDesc)', out.width = '100%', fig.align='center'}
set.seed(1994)
