...
In this section, we present the `DALEX` package for R, which covers the methods presented in this chapter. Similar functions can be found in packages `pdp` [@pdpRPackage], `ALEPlots` [@ALEPlotRPackage], and `iml` [@imlRPackage].
...
* `variables`, a character vector providing the names of the explanatory variables, for which the profile is to be computed; by default, `variables = NULL`, in which case computations are performed for all continuous [TOMASZ: CORRECT? SINCE VARIABLE_TYPE=NUMERICAL BY DEFAULT.] variables included in the model.
...
* `variable_type`, a character string indicating whether calculations should be performed only for `"numerical"`  	
(continuous) explanatory variables (default) or only for `"categorical"` variables;
...
The function that allows calculations of PD profiles is `model_profile()`. By defaut, it calculates profiles for all continuous variables. The other useful arguments include:

* `variables`, a character vector providing the names of the explanatory variables, for which the profile is to be computed; by default computations are performed for all continuous [TOMASZ: CORRECT? SINCE VARIABLE_TYPE=NUMERICAL BY DEFAULT.] variables included in the model.
...
* `B`, the numer of times (by default, 10) the entire procedure is to be repeated;
* `type`, the type of the PD profile, with values `'partial'` (default), `'conditional'`, and `'accumulated'`;
* `variable_type`, a character string indicating whether calculations should be performed only for `'numerical'`  	
(continuous) explanatory variables (default) or only for `'categorical'` variables;
...
In the example below, we calculate the PD profiles for *age* and *fare* by applying the `model_profile()` function to the explainer-object for the random-forest model. By default, the profile is based on 300 randomly selected observations. [TOMASZ: VARIABLES ARGUMENT ADDED FOR CONSISTENCY WITH THE PLOT. CHECK.] 

```{python,  eval = FALSE}
mp_rf = titanic_rf_exp.model_profile(variables = ['age', 'fare'])
mp_rf.result
```
...
The results can be visualised by applying the `plot()` method. Figure \@ref(fig:examplePythonMProfile2) presents the created plot. [TOMASZ: AGE AND FARE IN THE PLOT. CHECK FOR CONSISTENCY WITH THE SYNTAX.]

```{python,  eval = FALSE}
mp_rf.plot()
```

(ref:examplePythonMProfile2Desc) Partial-dependence profiles for *age* and *fare* for the random-forest model for the Titanic data, obtained by using the `plot()` method in Python. [TOMASZ: AGE AND FARE IN THE PLOT. CHECK FOR CONSISTENCY WITH THE SYNTAX.]
...
The resulting plot (not shown) is essentially the same as the one shown in the right-hand-side panel of Figure \@ref(fig:pdpIntuition). [TOMASZ: THIS IS NOT TRUE, I THINK. FIG 17.1 IS RELATED ONLY TO AGE. HERE, I UNDERSTAND, WE GET AGE AND FARE. WE SHOULD SHOW THE PLOT. REMEMBER TO ADD CAPTION AND REFERENCE.] 
...
The resulting plot is presented in Figure \@ref(fig:examplePythonMProfile3). [TOMASZ: HOW TO INTERPRET IT? WHAT IS THE HEIGT OF THE BARS? CAN IT BE LINKED TO A PLOT OF CP PROFILES EARLIER?]
...
The `model_profile()` function admits the `groups` argument that allows constructing PD profiles for groups of observations defined by the levels of an explanatory variable. In the code below, we use the argument to compute the profiles for *age* and *fare*, while grouping them by *gender*. Subsequently, we use the `plot()` method to obtain a graphical presentation of the results. The resulting plot is presented in Figure \@ref(fig:examplePythonMProfile4).
...
(ref:examplePythonMProfile4Desc) Partial-dependence profiles for *age* and *fare*, grouped by *gender*, for the random-forest model for the Titanic data, obtained by using the `plot()` method in Python. 
...
It may be of interest to compare PD profiles for several models. As an illustration, we will compare the random-forest model with the logistic-regression model `titanic_lr` (see Section \@ref(model-titanic-python-lmr)). First, we have got to compute to the profiles for both models by using the `model_profile()` function.
...
Subsequently, we apply the `plot()` method to plot the profiles. Note that, in the code below, we use the `variables` argument to limit the display to variable *age*.

```{python,  eval = FALSE}
pdp_rf.plot(pdp_lr, variables = ['age'])
```

As a result, the profiles for *age* are presented in a single plot. The resulting graph (not shown) is essentially the same as the one presented in Figure \@ref(fig:pdpPart7).
