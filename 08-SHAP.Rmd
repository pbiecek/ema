...
```{r shappJohny02, warning=FALSE, message=FALSE, echo=FALSE, fig.cap="Variable contributions for the prediction for `johny_d` for the random-forest model `titanic_rf_v6` and the Titanic data for 25 random orderings. Left plot: Box-plots summarize the distribution of the contributions for each explanatory variable across the orderings. Red and green bars present the SHAP values. Right plot: Average attributions without boxplots.", fig.width=11, fig.height=6, out.width = '100%', fig.align='center'}
library("patchwork")
...
In most applications the detailed information about the distribution of variable contributions across the considered orderings of explanatory variables will not be necessary. Thus, one could simplify the plot by presenting only the SHAP values, as in right panel in Figure \@ref(fig:shappJohny02). Table \@ref(tab:shapOrderingTable) presents the SHAP values underlying this plot. 
...
An important drawback of the SHAP values is that they are additive attributions of variable effects. If the model is not additive, then the SHAP values may be misleading. This issue can be seen as arising from the fact that, in the cooperative games, the goal is to distribute the payoff among payers. However, in the predictive modeling context, we want to understand how do the players affect the payoff? Thus, we are not limited to independent payoff-splits for players.
...
In this section, we present the key features of the `iBreakDown` R package [@iBreakDownRPackage] which is a part of the  `DrWhy.AI` universe. The package covers all methods presented in this chapter. 
...
To obtain a plot with only SHAP values, we can use the `show_boxplots=FALSE` argument in the `plot()` function call.
