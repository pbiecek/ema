...
Prediction level exlpainers help to understand factors beyond model responsses for individual data points.

Here we are asking questions related to a single models response, like:

- is the single prediction reliable


We are not interested in global model behaviour.

There are several usecases for such explainers. 

* Model improvement. If model works particualry bad for a selected observation (the residual is very high) then investigation of model responses for missfitted points may give some hints how to improve the model. For individual predictions it is easier to notice that selected variable should have different a effect.
* Increased trust in model predictions. 
* Model selection. Having multiple candidate responses for a single observation one may look for additional factors to select final decision. This moves us towards expert consilus that identify different options and allow user to seelct one of them. 
