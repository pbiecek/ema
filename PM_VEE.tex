...
## fired     2277  379      199   0.2024518
## ok         538 1245      438   0.4394417
## promoted   194  387     2190   0.2096716
\end{verbatim}
...
## Confusion matrix:
##          fired   ok promoted class.error
## fired     2269  386      200   0.2052539
## ok         531 1244      446   0.4398919
## promoted   201  389     2181   0.2129195
\end{verbatim}
...
## Predicted value: 0.830000, Average prediction: 0.374635 (diff = 0.455365) Predicted value: 0.168000, Average prediction: 0.276182 (diff = -0.108182) Predicted value: 0.002000, Average prediction: 0.349183 (diff = -0.347183)
## 
## Analysed predictor: 
...
## 1     gender fired  0.16696 0.10170921   gender=male
## 2        age fired  0.12300 0.10597378      age=57.7
## 3      hours fired  0.22398 0.11704174    hours=42.3
## 4 evaluation fired  0.02394 0.01170242  evaluation=2
## 5     salary fired -0.09146 0.02414552      salary=2
...
\hypertarget{LIME}{%
\chapter{Approximation with small model: Local model: LIME: Local
Interpretable Model-Agnostic Explanations}\label{LIME}}

Variable attribution method are not interested in the local curvature of
the model. They rarther compare model prediction agains average model
prediction.

The complementary approach would be to directly explore infromation
about model curvature around point of interest. In the section
\ref{ceterisParibus} we introduce Ceteris Paribus tool for what if
analysis. In this section we describe an another approach based on local
approximations with white-box models.

The most known paper related to local approximations is \emph{Why Should
I Trust You?: Explaining the Predictions of Any Classifier}
\citep{lime}. This methods and it's clones are now implemented in
variouse R and python packages, see for example \citep{R-lime} or
\citep{R-live}.
...
\NormalTok{lime_rf <-}\StringTok{ }\KeywordTok{lime}\NormalTok{(HR[,}\DecValTok{1}\OperatorTok{:}\DecValTok{5}\NormalTok{], model)}
\NormalTok{lime}\OperatorTok{::}\KeywordTok{explain}\NormalTok{(new_observation[,}\DecValTok{1}\OperatorTok{:}\DecValTok{5}\NormalTok{], lime_rf, }\DataTypeTok{n_labels =} \DecValTok{1}\NormalTok{, }\DataTypeTok{n_features =} \DecValTok{10}\NormalTok{)}


\KeywordTok{library}\NormalTok{(iml)}
\NormalTok{mod =}\StringTok{ }\NormalTok{Predictor}\OperatorTok{$}\KeywordTok{new}\NormalTok{(model, }\DataTypeTok{data =}\NormalTok{ HR[,}\DecValTok{1}\OperatorTok{:}\DecValTok{5}\NormalTok{])}
\NormalTok{x.interest =}\StringTok{ }\NormalTok{new_observation}
\NormalTok{lemon =}\StringTok{ }\NormalTok{LocalModel}\OperatorTok{$}\KeywordTok{new}\NormalTok{(mod, }\DataTypeTok{x.interest =}\NormalTok{ x.interest, }\DataTypeTok{k =} \DecValTok{5}\NormalTok{)}
\NormalTok{lemon}

\NormalTok{lemon}\OperatorTok{$}\NormalTok{results}
\KeywordTok{plot}\NormalTok{(lemon)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{data <-}\StringTok{ }\KeywordTok{mlbench.circle}\NormalTok{(}\DecValTok{1000}\NormalTok{)}
\NormalTok{data <-}\StringTok{ }\KeywordTok{mlbench.spirals}\NormalTok{(}\DecValTok{1000}\NormalTok{)}
\NormalTok{data <-}\StringTok{ }\KeywordTok{mlbench.2dnormals}\NormalTok{(}\DecValTok{1000}\NormalTok{)}
\NormalTok{df <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{x1 =}\NormalTok{ data}\OperatorTok{$}\NormalTok{x[,}\DecValTok{1}\NormalTok{], }\DataTypeTok{x2 =}\NormalTok{ data}\OperatorTok{$}\NormalTok{x[,}\DecValTok{2}\NormalTok{], }\DataTypeTok{class =}\NormalTok{ data}\OperatorTok{$}\NormalTok{classes)}

\KeywordTok{ggplot}\NormalTok{(df, }\KeywordTok{aes}\NormalTok{(x1, x2, }\DataTypeTok{color =}\NormalTok{ class)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{()}

\NormalTok{model_rf <-}\StringTok{ }\KeywordTok{randomForest}\NormalTok{(class }\OperatorTok{~}\StringTok{ }\NormalTok{., }\DataTypeTok{data =}\NormalTok{ df)}
\NormalTok{model_rf}

\NormalTok{new_obs <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{x1 =} \FloatTok{0.25}\NormalTok{, }\DataTypeTok{x2 =} \FloatTok{0.25}\NormalTok{, }\DataTypeTok{class =} \KeywordTok{factor}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DataTypeTok{levels =} \DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{))}
\NormalTok{new_obs <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{x1 =} \DecValTok{0}\NormalTok{, }\DataTypeTok{x2 =} \DecValTok{0}\NormalTok{, }\DataTypeTok{class =} \KeywordTok{factor}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DataTypeTok{levels =} \DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{))}
\KeywordTok{predict}\NormalTok{(model_rf, new_obs, }\DataTypeTok{type =} \StringTok{"prob"}\NormalTok{)}

\NormalTok{grid_df <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{x1 =} \KeywordTok{rep}\NormalTok{(}\KeywordTok{seq}\NormalTok{(}\OperatorTok{-}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{,}\FloatTok{0.01}\NormalTok{), }\DataTypeTok{each =} \DecValTok{201}\NormalTok{),}
                      \DataTypeTok{x2 =} \KeywordTok{rep}\NormalTok{(}\KeywordTok{seq}\NormalTok{(}\OperatorTok{-}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{,}\FloatTok{0.01}\NormalTok{), }\DataTypeTok{times =} \DecValTok{201}\NormalTok{))}

\NormalTok{grid_df}\OperatorTok{$}\NormalTok{y <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(model_rf, grid_df, }\DataTypeTok{type =} \StringTok{"prob"}\NormalTok{)[,}\DecValTok{1}\NormalTok{]}

\KeywordTok{ggplot}\NormalTok{(grid_df, }\KeywordTok{aes}\NormalTok{(x1, x2, }\DataTypeTok{color =}\NormalTok{ y)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{()}

\KeywordTok{library}\NormalTok{(lime)}
\NormalTok{model_type.randomForest <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(x, ...) }\StringTok{"classification"}
\NormalTok{lime_rf <-}\StringTok{ }\KeywordTok{lime}\NormalTok{(df[,}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{], model_rf)}
\NormalTok{lime}\OperatorTok{::}\KeywordTok{explain}\NormalTok{(new_obs[,}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{], lime_rf, }\DataTypeTok{n_labels =} \DecValTok{1}\NormalTok{, }\DataTypeTok{n_features =} \DecValTok{2}\NormalTok{)}


\KeywordTok{library}\NormalTok{(iml)}

\NormalTok{mod =}\StringTok{ }\NormalTok{Predictor}\OperatorTok{$}\KeywordTok{new}\NormalTok{(model_rf, }\DataTypeTok{data =}\NormalTok{ df)}
\NormalTok{x.interest =}\StringTok{ }\NormalTok{new_obs}
\NormalTok{lemon =}\StringTok{ }\NormalTok{LocalModel}\OperatorTok{$}\KeywordTok{new}\NormalTok{(mod, }\DataTypeTok{x.interest =}\NormalTok{ x.interest, }\DataTypeTok{k =} \DecValTok{2}\NormalTok{)}
\NormalTok{lemon}
...
\NormalTok{similar <-}\StringTok{ }\KeywordTok{sample_locally2}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ df,}
                           \DataTypeTok{explained_instance =}\NormalTok{ new_obs,}
                           \DataTypeTok{explained_var =} \StringTok{"class"}\NormalTok{,}
                           \DataTypeTok{size =} \DecValTok{500}\NormalTok{)}
\NormalTok{similar1 <-}\StringTok{ }\KeywordTok{add_predictions2}\NormalTok{(}\DataTypeTok{to_explain =}\NormalTok{ similar,}
                             \DataTypeTok{black_box_model =}\NormalTok{ model_rf, }
                              \DataTypeTok{predict_fun =} \ControlFlowTok{function}\NormalTok{(m,x) }\KeywordTok{predict}\NormalTok{(m,x,}\DataTypeTok{type=}\StringTok{"prob"}\NormalTok{)[,}\DecValTok{1}\NormalTok{])}
\NormalTok{wine_expl <-}\StringTok{ }\KeywordTok{fit_explanation2}\NormalTok{(}\DataTypeTok{live_object =}\NormalTok{ similar1,}
                              \DataTypeTok{white_box =} \StringTok{"regr.glm"}\NormalTok{)}

\KeywordTok{plot}\NormalTok{(wine_expl, }\DataTypeTok{type =} \StringTok{"forest"}\NormalTok{)}

\KeywordTok{library}\NormalTok{(ceterisParibus)}
\NormalTok{exp_rf <-}\StringTok{ }\NormalTok{DALEX}\OperatorTok{::}\KeywordTok{explain}\NormalTok{(model_rf, df, }\DataTypeTok{predict_function =} \ControlFlowTok{function}\NormalTok{(m,x) }\KeywordTok{predict}\NormalTok{(m,x,}\DataTypeTok{type=}\StringTok{"prob"}\NormalTok{)[,}\DecValTok{1}\NormalTok{])}

\NormalTok{cp_rf <-}\StringTok{ }\KeywordTok{ceteris_paribus}\NormalTok{(exp_rf, new_obs)}
\KeywordTok{plot}\NormalTok{(cp_rf)}

\NormalTok{cp2_rf <-}\StringTok{ }\KeywordTok{what_if_2d}\NormalTok{(exp_rf, new_obs)}
\KeywordTok{plot}\NormalTok{(cp2_rf)}
\end{Highlighting}
\end{Shaded}
...
## 
##           Mean of squared residuals: 488206.6
##                     % Var explained: 40.55
\end{verbatim}
...
## 1                1920        5     142     8 3089.915 construction.year
## 1.1              1921        5     142     8 3113.064 construction.year
## 1.2              1922        5     142     8 3116.172 construction.year
## 1.3              1923        5     142     8 3074.531 construction.year
## 1.4              1923        5     142     8 3074.531 construction.year
## 1.5              1924        5     142     8 3073.167 construction.year
##     _ids_      _label_
## 1       1 randomForest
## 1.1     1 randomForest
## 1.2     1 randomForest
...
## 1              1965        5     142     8 2351.078 randomForest
\end{verbatim}
...
## 2           surface     1     619.1496
## 4          no.rooms     1     446.8994
## 3             floor     1     299.3370
## 1 construction.year     1     233.8213
\end{verbatim}
...
\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-29-1.pdf}
...
\includegraphics{PM_VEE_files/figure-latex/unnamed-chunk-30-1.pdf}
