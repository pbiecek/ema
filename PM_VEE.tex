...
one refers to ,,explanatory variables,`' with ,,independent
variables,'`,,predictors,'`or ,,covariates'`as often-used equivalents.
Explanatory variables are used in the model as means to explain
(predict) the ,,dependent variable,'`also called ,,predicted'`variable
or ,,response.'`In the machine-learning language, ,,input variables'`or
,,features'`are used to predict the ,,output'`variable. In statistical
modelling, models are fit to the data that contain
,,observations,'`whereas in the machine-learning world a dataset may
contain ,,instances.''
...
understand how changes in the model input affects changes in the model
output. Sometimes these techniques are called sensitivity analysis.
...
In next sections we introduce 1D and 2D Ceteris Profiles more formally.
Here we present profiles for continuous variable, but they can be easily
generalized for variables of other types, like line, color or
categorical variables.

\subsubsection{1D profiles}\label{ceterisParibus1d}

Let \(f(x): \mathcal R^{d} \rightarrow \mathcal R\) denote a predictive
model, i.e.~function that takes \(d\) dimensional vector and calculate
numerical score. Symbol \(x \in \mathcal R^d\) refers to a point in the
feature space. We use subscript \(x_i\) to refer to a different data
points and superscript \(x^j\) to refer to specific dimensions/features.
Additionally, let \(x^{-j}\) denote all coordinates except \(j\)-th and
let \(x|^j=z\) denote a data point \(x\) with all coordinates equal to
\(x\) except for a coordinate \(j\) equal to value \(z\). I.e.
...
Usually the model input consist many variables. CP profiles are readable
even for tiny subplots, what makes them perfect for the small multiples.
This way we can analysis more variables at the same time. The easiest
way to do so is to plot consecutive variables on separate panels. See an
example in Figure \ref{fig:HRCPFiredAll}.
...
large number of features we end up with large number of panels. In order
to prioritize panels that are more important we need a measure that
assess an impact of a selected feature on model output.

Profile oscillations are one of such measures. They can be used for
proper ordering of CP profiles for important features or for filtering
of feature of interest. CP profiles lead to a very natural and intuitive
way of assessing the local feature importance for a single prediction.
The intuition is: the more important is a feature the larger are
fluctuations along its CP profile. If feature is not important then
...
Let \(vip^{CP}_j(x)\) denotes local variable importance calculated based
on a CP profile for feature \(j\) in a point \(x\). Let \(g^j(z)\)
stands for a density of feature \(j\) then expected absolute oscilations
are defined as
...
So it's an area between \(f(x)\) and Ceteris Paribus profile weighted by
density of feature \(j\). Note that one can consider different
modification of this measure, for example deviations can be calculated
not as a distance from \(f(x)\) but from average \(\bar CP^{f,j,x}(z)\)
or instead of absolute deviations one may use root from average squares.
...
A very interesting approach for model explorartion with similar
principles is implemented in the \texttt{condvis} package
\citep{JSSv081i05}.
...
Ceteris Paribus profiles with \texttt{ingredients} package are
...
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(}\StringTok{"DALEX"}\NormalTok{)}
\NormalTok{explainer_rf <-}\StringTok{ }\KeywordTok{explain}\NormalTok{(rf_model, }\DataTypeTok{data =}\NormalTok{ apartmentsTest, }\DataTypeTok{y =}\NormalTok{ apartmentsTest}\OperatorTok{$}\NormalTok{m2.price)}
\end{Highlighting}
\end{Shaded}
...
## 1                1920        5     142     8 3076.926 construction.year
## 1.1              1921        5     142     8 3110.971 construction.year
## 1.2              1922        5     142     8 3098.656 construction.year
## 1.3              1923        5     142     8 3042.993 construction.year
## 1.4              1924        5     142     8 3054.198 construction.year
## 1.5              1925        5     142     8 3072.960 construction.year
##     _ids_      _label_
## 1       1 randomForest
## 1.1     1 randomForest
## 1.2     1 randomForest
...
## 1              1965        5     142     8 2324.303 randomForest     1
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(cp_rf) }\OperatorTok{+}
\StringTok{  }\KeywordTok{show_observations}\NormalTok{(cp_rf)}
\end{Highlighting}
\end{Shaded}
...
## 3           surface     1     623.4693
## 2          no.rooms     1     426.2384
## 4             floor     1     325.6314
## 1 construction.year     1     244.2677
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{wi_rf_2d <-}\StringTok{ }\KeywordTok{ceteris_paribus_2d}\NormalTok{(explainer_rf, }\DataTypeTok{observation =}\NormalTok{ new_apartment, }
...
## ok         523 1258      440   0.4335885
## promoted   205  389     2177   0.2143630
\end{verbatim}
...
\begin{verbatim}
##                                       contribution
## randomForest.fired: intercept                0.375
## randomForest.fired: hours = 42               0.240
## randomForest.fired: evaluation = 2           0.063
## randomForest.fired: salary = 2              -0.243
## randomForest.fired: gender = male            0.039
## randomForest.fired: age = 58                 0.367
## randomForest.fired: prediction               0.840
## randomForest.ok: intercept                   0.276
## randomForest.ok: hours = 42                 -0.050
## randomForest.ok: evaluation = 2              0.091
## randomForest.ok: salary = 2                  0.242
## randomForest.ok: gender = male              -0.042
## randomForest.ok: age = 58                   -0.357
## randomForest.ok: prediction                  0.160
## randomForest.promoted: intercept             0.350
...
## randomForest.promoted: age = 58             -0.011
...
## Confusion matrix:
##          fired   ok promoted class.error
## fired     2259  394      202   0.2087566
## ok         525 1249      447   0.4376407
## promoted   204  386     2181   0.2129195
\end{verbatim}
...
## randomForest.fired: salary = 2                    -0.206
## randomForest.fired: age:gender = 58:male           0.394
## randomForest.fired: evaluation = 2                -0.017
## randomForest.fired: prediction                     0.784
## randomForest.ok: intercept                         0.276
## randomForest.ok: evaluation = 2                    0.136
## randomForest.ok: salary = 2                        0.154
## randomForest.ok: age:gender = 58:male             -0.287
## randomForest.ok: hours = 42                       -0.067
## randomForest.ok: prediction                        0.212
...
\NormalTok{x_train <-}\StringTok{ }\NormalTok{HR[ , }\OperatorTok{-}\DecValTok{6}\NormalTok{]}
...
## 1     gender fired  0.19342 0.10799558   gender=male
## 2        age fired  0.10742 0.09539542      age=57.7
## 3      hours fired  0.18758 0.10391857    hours=42.3
## 4 evaluation fired  0.03462 0.01595480  evaluation=2
## 5     salary fired -0.12202 0.03478881      salary=2
## 6     gender    ok -0.18100 0.09141863   gender=male
\end{verbatim}
...
## Confusion matrix:
##          fired   ok promoted class.error
## fired     2271  389      195   0.2045534
## ok         530 1249      442   0.4376407
## promoted   197  384     2190   0.2096716
\end{verbatim}
...
## 1 classification    1    fired      0.800 0.1226461       0.2576573
## 2 classification    1    fired      0.800 0.1226461       0.2576573
## 3 classification    1    fired      0.800 0.1226461       0.2576573
## 4 classification    1       ok      0.198 0.1319438       0.1930195
## 5 classification    1       ok      0.198 0.1319438       0.1930195
## 6 classification    1       ok      0.198 0.1319438       0.1930195
## 7 classification    1 promoted      0.002 0.2865882       0.5223019
## 8 classification    1 promoted      0.002 0.2865882       0.5223019
## 9 classification    1 promoted      0.002 0.2865882       0.5223019
##   model_prediction    feature feature_value feature_weight
## 1      0.616120792        age          57.7   0.0066489544
## 2      0.616120792      hours          42.3   0.2497250401
## 3      0.616120792 evaluation           2.0   0.1020894584
## 4      0.476270872        age          57.7   0.0003716683
## 5      0.476270872 evaluation           2.0   0.2068034186
## 6      0.476270872     salary           2.0   0.0760762580
## 7      0.001276047        age          57.7  -0.0053801294
## 8      0.001276047 evaluation           2.0  -0.3098289456
## 9      0.001276047      hours          42.3  -0.2058167373
##           feature_desc                      data          prediction
## 1           50.0 < age 2.0, 57.7, 42.3, 2.0, 2.0 0.800, 0.198, 0.002
## 2 37.6 < hours <= 46.3 2.0, 57.7, 42.3, 2.0, 2.0 0.800, 0.198, 0.002
## 3      evaluation <= 3 2.0, 57.7, 42.3, 2.0, 2.0 0.800, 0.198, 0.002
## 4           50.0 < age 2.0, 57.7, 42.3, 2.0, 2.0 0.800, 0.198, 0.002
## 5      evaluation <= 3 2.0, 57.7, 42.3, 2.0, 2.0 0.800, 0.198, 0.002
## 6      1 < salary <= 2 2.0, 57.7, 42.3, 2.0, 2.0 0.800, 0.198, 0.002
## 7           50.0 < age 2.0, 57.7, 42.3, 2.0, 2.0 0.800, 0.198, 0.002
## 8      evaluation <= 3 2.0, 57.7, 42.3, 2.0, 2.0 0.800, 0.198, 0.002
## 9 37.6 < hours <= 46.3 2.0, 57.7, 42.3, 2.0, 2.0 0.800, 0.198, 0.002
\end{verbatim}
...
## 1  0.06002809       1.0  0.06002809       male gender=male   gender=male
## 2  0.00524879      57.7  0.30285520       57.7         age      age=57.7
## 3 -0.08925678      42.3 -3.77556181       42.3       hours    hours=42.3
## 4 -0.49854398       2.0 -0.99708796          2  evaluation  evaluation=2
## 5 -0.03377847       2.0 -0.06755694          2      salary      salary=2
## 6 -0.03083929       1.0 -0.03083929       male gender=male   gender=male
##   .class
## 1  fired
## 2  fired
## 3  fired
...
\NormalTok{data_old =}\StringTok{ }\NormalTok{HR[HR}\OperatorTok{$}\NormalTok{gender }\OperatorTok{==}\StringTok{ "male"}\NormalTok{, }\OperatorTok{-}\DecValTok{1}\NormalTok{]}
\NormalTok{data_new =}\StringTok{ }\NormalTok{HR[HR}\OperatorTok{$}\NormalTok{gender }\OperatorTok{==}\StringTok{ "female"}\NormalTok{, }\OperatorTok{-}\DecValTok{1}\NormalTok{]}
