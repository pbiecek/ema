...
```
 ...
 Leo Breiman in his work Two Cultures [@twoCultures] described a phenomenon called Rashomon effect. It means that several models with similar performance can base their predictions on a completely different relations extracted from the same data.

This is the case with four models presented in this section. Although they are all quite effective, for one particular observation, here Johnny D, they give different predictions and different explanations.

We can still read a lot from this jungle of explanations. For example, Ceteris-paribus profiles for GBM techniques generate curves with very high variability (see Figure \@ref(championChallengerCP)). Such a high fluctuation suggests an overfitting, so I would rather not use this model.
