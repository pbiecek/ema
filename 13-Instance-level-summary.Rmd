...
set.seed(1313)
sp_rf_johny <- shap(explain_titanic_rf, johny_d)
sp_gbm_johny <- shap(explain_titanic_gbm, johny_d)
sp_svm_johny <- shap(explain_titanic_svm, johny_d)
sp_lmr_johny <- shap(explain_titanic_lmr, johny_d)
...
set.seed(1)
bd_rf_johny <- break_down(explain_titanic_rf, johny_d, interactions = TRUE)
bd_gbm_johny <- break_down(explain_titanic_gbm, johny_d, interactions = TRUE)
bd_svm_johny <- break_down(explain_titanic_svm, johny_d, interactions = TRUE)
bd_lmr_johny <- break_down(explain_titanic_lmr, johny_d, interactions = TRUE)
...
```{r championChallengerCP, warning=FALSE, message=FALSE, echo=FALSE, fig.width=9, fig.height=5, fig.cap="Ceteris Paribus profiles for four different models for the Titanic data.", out.width = '80%', fig.align='center'}
splits <- list(age = seq(0,70,0.1), fare = seq(0,100,0.2))

cp_rf_johny <- ceteris_paribus(explain_titanic_rf, johny_d, variable_splits = splits)
cp_gbm_johny <- ceteris_paribus(explain_titanic_gbm, johny_d, variable_splits = splits)
cp_svm_johny <- ceteris_paribus(explain_titanic_svm, johny_d, variable_splits = splits)
cp_lmr_johny <- ceteris_paribus(explain_titanic_lmr, johny_d, variable_splits = splits)

plot(cp_rf_johny, cp_gbm_johny, cp_svm_johny, cp_lmr_johny, 
     color = "_label_") +
  show_observations(cp_rf_johny, cp_gbm_johny, cp_svm_johny, cp_lmr_johny, 
                    variables = c("age", "fare"), size = 3) +
