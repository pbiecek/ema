...
```{r BDPrice4, echo=FALSE, fig.cap="Break-down plots show how the contribution of individual explanatory variables change the average model prediction to the prediction for a single instance (observation). Panel A) The first row shows the distribution and the average (red dot) of model predictions for all data. The next rows show the dirstribution and the average of the predictions when fixing values of subseqeunt explanatory variables. The last row shows the prediction for a particular instance of interest. B) Red dots indicate the average predictions from Panel B. C) The green and red bars indicate, resspectively, positive and negative changes in the average predictions (variable contributions). ", out.width = '70%', fig.align='center'}
...
(\#eq:single_break_down_contribution)
\end{equation}
...
(\#eq:general_break_down_local_accuracy)
\end{equation}
...
(\#eq:general_break_down_proposition)
\end{equation}
...
```{r BDjohnyExample, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Break-down plot for the `titanic_rf_v6` model and `johny_d` for the Titanic data.", out.width = '70%', fig.align='center'}
library("iBreakDown")
...
In this section, we present key features of the `iBreakDown` R package [@iBreakDownRPackage] which is a part of the  `DrWhy.AI` universe. The  package covers all methods presented in this chapter. It is available on CRAN and GitHub. 
...
Applying the generic `plot()` function to the object resulting from the application of the `break_down()` function creates a BD plot. In this case, it is the plot from Figure \@ref(fig:BDhenryExample).

```{r BDhenryExample, warning=FALSE, message=FALSE,  fig.cap="Generic plot() function for the break down method for `henry`.", out.width = '70%', fig.align='center', fig.width=8, fig.height=5.5}
plot(bd_rf) 
```
...
```{r, warning=FALSE, message=FALSE,  fig.cap="Break Down plot limited to top three features.", out.width = '70%', fig.align='center', fig.width=8, fig.height=5.5}
library("iBreakDown")
...
```{r, warning=FALSE, message=FALSE,  fig.cap="Distributions for conditional conditionings.", out.width = '70%', fig.align='center', fig.width=8, fig.height=5.5}
