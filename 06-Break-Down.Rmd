...
\begin{eqnarray}
f(\underline{x}_*) &=& \left\{\beta^0 + {\beta}^1E_{X^1}(X^1) + ... + {\beta}^pE_{X^p}(X^p)\right\}+\\ &+& \left[\left\{{x}^1_* - E_{X^1}(X^1)\right\} {\beta}^1 + ... +\left\{{x}^p_* - E_{X^p}(X^p)\right\} {\beta}^p\right]\\ &\equiv& (mean \ prediction) + \sum_{j=1}^p v(j, \underline{x}_*).
(\#eq:singleBreakDownResult)
\end{eqnarray}
...
The explainer object allows a uniform access to predictive models regardless of their internal structure. With this object, we can proceed to the model analysis.
...
To obtain a waterfall chart we can simply the `plot()` method. It generates an interactive chart based on the `plotly` library.
...
(ref:bdPython1Desc) Break-down plot for the random-forest model and Henry for the Titanic data, obtained by the `plot()` method in Python. [TOMASZ: CHERBOURG]

```{r bdPython1, echo=FALSE, out.width = '80%', fig.align='center', fig.cap='(ref:bdPython1Desc)'}
...
(ref:bdPython2Desc) Break-down plot for a limited number of explanatory variables in a specified order for the random-forest model and Henry for the Titanic data, obtained by the `plot()` method in Python. [TOMASZ: CHERBOURG]

```{r bdPython2, echo=FALSE, out.width = '90%', fig.align='center', fig.cap='(ref:bdPython2Desc)'}
