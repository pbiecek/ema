...
Table: (\#tab:titanicBreakDownDeltas) Expected values $E_{\underline{X}}\{f(\underline{X}) | X^j = {x}^j_*\}$ and scores $|\Delta^{j|\emptyset}(\underline{x}_*)|$ for the random-forest model *titanic rf* and Johnny D for the Titanic data. The scores are sorted in the decreasing order.
...
(ref:BDhenryExampleDesc) Break-down plot obtained by the generic `plot()` function for the random-forest model and Henry for the Titanic data.

```{r BDhenryExample, warning=FALSE, message=FALSE,  fig.cap='(ref:BDhenryExampleDesc)', out.width = '80%', fig.align='center', fig.width=7, fig.height=4}
...
(ref:BDhenryExampleTopDesc) Break-down plot for top three variables for the random-forest model and  Henry for the Titanic data.
...
bd_henry = titanic_rf_exp.predict_parts(henry, 
            type = 'break_down',
            order = np.array([0, 2, 1, 3, 4, 5, 6]))
bd_henry.plot(max_vars = 5)
```

```{r bdPython2, echo=FALSE, out.width = '90%', fig.align='center'}
