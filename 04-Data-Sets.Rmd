...
See an example of this dataset in the Table below.
...
The result is a model  stored in object `model_lr_v6`, which will be used in subsequent chapters. 
 
```{python, eval=FALSE}
from sklearn.linear_model import LogisticRegression

model_lr_v6 = make_pipeline(
    preprocess,
...
The result is a model  stored in object `model_rf_v6`, which will be used in subsequent chapters. 

```{python, eval=FALSE}
from sklearn.ensemble import RandomForestClassifier

model_rf_v6 = make_pipeline(
    preprocess,
...
The result is a model  stored in object `model_xgb_v6`, which will be used in subsequent chapters. 

```{python, eval=FALSE}
from sklearn.ensemble import GradientBoostingClassifier

model_xgb_v6 = make_pipeline(
    preprocess,
    GradientBoostingClassifier(n_estimators=100))

model_xgb_v6.fit(X, y)
```
...
Finally, we also consider a support-vector-machine model [@svm95vapnik]. We use the C-Support Vector Classification mode.
 To fit the model, we use `SVC` algorithm from `sklearn` library.
...
model_svm_v6 = make_pipeline(
    preprocess,
    SVC(probability=True))
    
model_svm_v6.fit(X, y)
```
...
Subsequently, we use the generic method `predict_proba()` to obtain the predicted probability of survival for the logistic-regression model. 

```{python, eval=FALSE}
model_lr_v6.predict_proba(johnny_d)
...
```{python, eval=FALSE}
model_rf_v6.predict_proba(johnny_d)
# array([[0.63028556, 0.36971444]])

model_xgb_v6.predict_proba(johnny_d)
# array([[0.1567194, 0.8432806]])

model_svm_v6.predict_proba(johnny_d)
...
model_lr_v6.predict_proba(henry)
# array([[0.69547744, 0.30452256]])

model_rf_v6.predict_proba(henry)
# array([[0.73060059, 0.26939941]])

model_xgb_v6.predict_proba(henry)
# array([[0.1567194, 0.8432806]])

model_svm_v6.predict(henry)
# array([[0.82429369, 0.17570631]])
```

### Models' explainers {#ExplainersTitanicPythoCode}
...
* `predict_function`, a function that returns prediction scores; if not specified, then a default `predict_proba()` function is used; 
* `residual_function`, a function that returns model residuals; 
* `verbose`, a logical argument (`verbose = TRUE` by default) indicating whether diagnostic messages are to be printed; 
* `model_info`, a named list (with components "package," "version," and "type") providing information about the model; if not specified `dalex` seeks for information on its own;
...
```{python, eval=FALSE}
exp_rf_v6 = dx.Explainer(model_rf_v6, X, y, label = "Titanic RF Pipeline")
exp_lr_v6 = dx.Explainer(model_lr_v6, X, y, label = "Titanic LR Pipeline")
exp_xgb_v6 = dx.Explainer(model_xgb_v6, X, y, label = "Titanic XGB Pipeline")
exp_svm_v6 = dx.Explainer(model_svm_v6, X, y, label = "Titanic SVM Pipeline")
```
...
## Python regression model for Apartment prices
