...
The first dataset will be used to illustrate the application of the techniques in the case of classification predictive models for a binary dependent variable. This dataset is mainly used in the examples in part II of this book.
The second one will offer an example of regression models for a continuous variable. This dataset is mainly used in the examples in part III of this book. 
...
Figures \@ref(fig:titanicExplorationGenderAge)--\@ref(fig:titanicExplorationCountryHarbor) present graphically the proportion of non- and survivors for different levels of the other variables with the use of mosaic plots. The width of the bars (on the x-axis) reflects the marginal distribution (proportions) of the observed levels of the variable. On the other hand, the height of the bars (on the y-axis) provides information about the proportion of non- and survivors. The graphs for *age* and *fare* were constructed by using the categorized versions of the variables.
...
## Classification models for Titanic, snippets for R
...
Table: (\#tab:archivistHooksOfModelsTitanic) Predictive models created for the `titanic` dataset. All models are trained with following variables: *gender*, *age*, *class*, *sibsp*, *parch*, *fare*, *embarked.*
...
Table: (\#tab:archivistHooksOfDataFramesTitanic) Data frames created for the Titanic use-case. All frames have following variables: *gender*, *age*, *class*, *embarked*, *country*, *fare*, *sibsp*, *parch*. The `titanic` has also `survived` variable.
...
## Classification models for Titanic, snippets for  Python
...
The result is a model stored in object `titanic_rf`, which will be used in subsequent chapters. 
...
The result is a model  stored in object `titanic_gbc`, which will be used in subsequent chapters. 
...
The result is a model  stored in object `titanic_svm`, which will be used in subsequent chapters. 
...
* `data`, a data frame or matrix providing data to which the model is to be applied. It should be an object of the `pandas.DataFrame` class, otherwise it will be converted to `pandas.DataFrame`. 
* `y`, values of the dependent variable/target variable corresponding to the data given in the `data` object; 
* `predict_function`, a function that returns prediction scores; if not specified, then `dalex` will make a guess which function shall be used, `predict()`, `predict_proba()` or something else; Note that this function shall work on `pandas.DataFrame` objects. If the `predict()` function works only for `numpy` matrices then apropriate conversion shall be included in `predict_function`;
...
titanic_gbc_exp = dx.Explainer(titanic_gbc, 
                    X, y, label = "Titanic XGB Pipeline")
...
## Regression models for Apartment prices, snippets for R
...
Finally, we consider a support vector machine model, with all the variables in the `apartments` data frame other than *m2.price* treated as explanatory. To fit the model, we use the `svm()` function, with default settings, from package `e1071` [@e1071]. The results of the model are stored in model-object `apartments_svm`. 
...
Table: (\#tab:archivistHooksOfModelsApartments) Predictive models created for the dataset Apartment prices. All models are trained with variables *construction.year*, *surface*, *floor*, *no.rooms*, *district*.
...
## Regression models for Apartment prices, snippets for  Python

Examples in Python are based on the `apartments` data set, which is available in the `dalex` library. The `m2_price` column is a target variable, the remaining columns will be used to construct the predictive model.

The following instructions load the `apartments` dataset and split it into the target variable `y` and the predictive variables `X`. 
...
As an alternative to linear regression, we consider a random-forest model. Again, we treat all the variables in the `apartments` data frame other than `m2_price` as explanatory and include them in the model. 
