...
We will illustrate the methods presented in this book by using two datasets on: 
   
* predicting odds of survival for passangers of the *RMS Titanic*; 
...
* missing `fare` is replaced by the within-`class` average, i.e., 89, 22, and 13:
...
Figures \@ref(fig:titanicExplorationGenderAge)-\@ref(fig:titanicExplorationCountry) present graphically the proportion non- and survivors for different levels of the other variables with the use of mosaic plots. The height of the bars (on the y-axis) reflects the marginal distribution (proportions) of the observed levels of the variable. On the other hand, the width of the bars (on the x-axis) provides the information about the proportion of non- and survivors. The graphs for *age* and *fare* were constructed by usingthe categorized versions of the variables.

Figure \@ref(fig:titanicExplorationGenderAge) indicates that the proportion of survivors was larger for females and children below 5 years of age. This is most likely the result of the "women and children first" principle that is often evoked in situations that require evacuation of persons whose life is in danger. The principle can, perhaps, partially explain the trend seen in Figure \@ref(fig:titanicExplorationParch), i.e., a higher proportion of survivors among those with 1-2 parents/children and 1-2 siblings/spouses aboard. Figure \@ref(fig:titanicExplorationClassFare) indicates that passengers travelling in the first and second class had a higher chance of survival, perhaps due to the proximity of the location of their cabins to the deck. Interestingly, the proportion of survivors among crew deck was similar to the proportion of the first-class passengers. The figure also shows that the proportion of survivors increased with the fare, which is consistent with the fact that the proportion was higher for passengers travelling in the first and second class. Finally, Figure \@ref(fig:titanicExplorationCountryHarbor) does not suggest any noteworthy trends.        

```{r titanicExplorationHistograms, warning=FALSE, message=FALSE, echo=FALSE, fig.width=11, fig.height=6, fig.cap="Histograms for age and fare for the Titanic data.", out.width = '100%', fig.align='center'}
library("ggplot2")
library("ggmosaic")
library("patchwork")
library("forcats")
...
```{r titanicExplorationGenderAge, warning=FALSE, message=FALSE, echo=FALSE, fig.width=11, fig.height=6, fig.cap="Survival status in the groups defined be gender and age category for the Titanic data.", out.width = '100%', fig.align='center'}
...
```{r titanicExplorationClassFare, warning=FALSE, message=FALSE, echo=FALSE, fig.width=11, fig.height=6, fig.cap="Survival according to class and fare in the Titanic data.", out.width = '100%', fig.align='center'}
...
```{r titanicExplorationCountryHarbor, warning=FALSE, message=FALSE, echo=FALSE, fig.width=11, fig.height=6, fig.cap="Survival according to the embarked harbor and country in the Titanic data.", out.width = '100%', fig.align='center'}
...
The dependent variable of interest, *survival*, is binary. Thus, a natural choice is to start the predictive modelling with a logistic-regression model. As there is no reason to expect a linear relationship between age and odds of survival, we use linear tail-restricted cubic splines, available in the `rcs()` function of the `rms` package [@rms], to model the effect of age. We also do not expect linear relation for the `fare` variable, but because of it's skewness (see Figure \@ref(fig:titanicExplorationHistograms)), we do not use splines for this variable. The results of the model are stored in model-object `titanic_lmr_v6`, which will be used in subsequent chapters. 
...
Finally, we consider also a support-vector-machine model [@svm95vapnik]. We use the C-classification mode. Again, we fit a model with the same set of explanatory variables as the logistic-regression model (see Section \@ref(model-titanic-lmr)) To fit the model, we use function `svm()` from the `e1071` package [@e1071]. The results of the model are stored in model-object `titanic_svm_v6`.
...
Let us now compare predictions that are obtained from the different models. In particular, we will compute the predicted probability of survival for an 8-year-old boy who embarked in Southampton and travelled in the 1st class with no parents nor siblings, and with a ticket costing 72 pounds. 
...
We do the same for the random-forest and gradient-boosting models. [TOMASZ: WHAT ABOUT SVM? IT WILL BE USEFUL IN THE SUMMARY CHAPTER OF THE INSTANCE-LEVEL PART.]
...
The models lead to different probabilities. Thus, it might be of interest to understand the reason for the differences, as it could help us to decide which of the predictions we might want to trust. We will investigate this issue in the subsequent chapters.

Note that, for some examples later in the book, we will use another observation (instance) with lower chances of survival. Let's call this passenger Henry.
...
### Model explainers {#ExplainersTitanicRCode}

Model-objects created with different libraries may have different internal structures. Thus, first, we have got to create an "explainer" that provides an uniform interface for different models. Toward this end, we use the `explain()` function from the `DALEX` package [@DALEX]. The function requires six arguments: 

* `model`, a model-object;
* `data`, a data frame or matrix used to fit the model; note that the data object should not contain the dependent variable;     
...
* `residual_function`, a function that returns model residuals; if not specified, then model residuals defined in equation \@ref(eq:modelResiduals) are calculated;
* `label`, an unique name of the model; if not specified, then it is extracted from the `class(model)`. 

Each explainer contains all elements needed to create a model explanation, i.e., a suitable `predict()` function, the data, and the model object. Thus, to keep code snippets more concise, in subsequent chapters we will use the explainers instead of the model objects. 
...
Table \@ref(tab:archivistHooksOfDataFramesTitanic) summarizes the dataframes that will be used in examples in the subsequent chapters.

Table: (\#tab:archivistHooksOfDataFramesTitanic) Dataframes created for the `titanic` example. 
...
* *m2.price*, apartment's price per meter-squared (in EUR), a numerical variable in the range of 1607 -- 6595;
...
The R code below provides more information about the contents of the dataset, values of the variables, etc.
...
Model predictions will be obtained for a set of the first six apartments included in dataframe `apartments_test`.
...
ggplot(data = apartments) +
   ...
   labs(x="Price per meter-squared", title='Histogram of apartment price') + 
...
```{r appartmentsMi2Construction, warning=FALSE, message=FALSE, echo=FALSE, fig.width=11, fig.height=6, fig.cap="Apartments data. Left-hand-side panel shows apartment price per meter-squared vs. year of construction, right-hand-side panel shows price  vs. surface.", out.width = '100%', fig.align='center'}
...
Figure \@ref(fig:appartmentsMi2Floor) indicates that relation between *floor* and *m2.price* is also close to linear, as well as relation between *no.rooms* and *m2.price* .

```{r appartmentsMi2Floor, warning=FALSE, message=FALSE, echo=FALSE, fig.width=11, fig.height=6, fig.cap="Apartments data. Price per meter-squared vs. floor (left-hand-side panel) and vs. number of rooms (right-hand-side panel).", out.width = '100%', fig.align='center'}
...
Figure \@ref(fig:appartmentsSurfaceNorooms) shows that *surface* and *number of rooms* are correlated and that prices depend on district. In particular, boxplots in Figure \@ref(fig:appartmentsSurfaceNorooms) indicate that the highest prices per meter-squared are observed in Srodmiescie (Downtown).

```{r appartmentsSurfaceNorooms, warning=FALSE, message=FALSE, echo=FALSE, fig.width=11, fig.height=6, fig.cap="Apartments data. Left-hand-side panel shows surface vs. number of rooms, right-hand-side panel shows price per meter-squared for different districts.", out.width = '100%', fig.align='center'}
pa5 <- ggplot(data = apartments, aes(no.rooms, surface, group = no.rooms)) +
    geom_boxplot(aes(group = no.rooms), se = FALSE, size=0.5, fill = "#371ea3", color = "white", alpha=0.3) +
   geom_jitter(size = 0.3, width = 0.15, height = 0) +
   labs(y="Surface (meter-squared)", x = "Number of rooms", title='Relation between surface and # of rooms') + theme_drwhy() + theme(legend.position = "none")  + scale_x_continuous(breaks = 1:6)
...
The dependent variable of interest, *m2.price*, is continuous. Thus, a natural choice to build a predictive model is linear regression. We treat all the other variables in the `apartments` dataframe as explanatory and include them in the model. To fit the model, we apply the `lm()` function. The results of the model are stored in model-object `apartments_lm_v5`.
...
As an alternative to linear regression, we consider a random-forest model. Again, we treat all the variables in the `apartments` dataframe other than *m2.price* as explanatory and include them in the model. To fit the model, we apply the `randomForest()` function, with default settings, from the package with the same name [@randomForest].  
...
Finally, we consider a support-vector-machine model, with all the variables in the `apartments` dataframe other than *m2.price* treated as explanatory. To fit the model, we use the `svm()` function, with default settings, from the package `e1071` [@R-e1071]. The results of the model are stored in model-object `apartments_svm_v5`. 
...
The `predict()` function calculates predictions for a specific model. In the example below, we use model-objects `apartments_lm_v5` and `apartments_rf_v5` to calculate predictions for the linear-regression and random-forest model, respectively, for prices for the first six observations from the `apartments` dataframe. [TOMASZ: HOW ABOUT SVM?]
...
For the random-forest model, RMSE is equal to `r round(rmsd_rf, 1)`. It is almost identical to the RMSE for the linear-regression model, which is equal to `r round(rmsd_lm, 1)`. [TOMASZ: SOMETHING IS WRONG HERE? I AM GETTING VALUES 283 AND 795, WHICH ARE NOT THE SAME! BUT MSES IN THE OUTPUTS ARE 79131 AND 79789.39, WITH RMSE 281.3 AND 282.5.] Thus, the question we may face is: should we choose the more complex, but flexible random-forest model, or the simpler and easier to interpret linear-regression model? In the subsequent chapters we will try to provide an answer to this question. In particular, we will show that a proper model exploration may help to discover weak and strong sides of aby of the models and, in consequence, allow creation of a new model, with a better performance than either of the two.

### Model adapters {#ExplainersApartmentsRCode}

For the reasons outlined in Section \@ref(ExplainersTitanicRCode) for the Titanic dataset, we will also use explainers for the predictive models created for the `apartments` dataset.
...
In Sections \@ref(model-Apartments-lr) and \@ref(model-Apartments-rf), we have built two  [TOMASZ: HOW ABOUT SVM?] predictive models for the `apartments` data set. The models will be used in the rest of the book to illustrate the model explanation methods and tools. 
