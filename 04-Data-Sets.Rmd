...
explain_titanic_gbm_v6 <- explain(model = titanic_gbm_v6, 
                                 data = titanic[, -9],
                                 y = titanic$survived == "yes", 
                                 predict_function = function(m, x) predict(m, x, n.trees = 100), # TODO: is it still needed?
...
```{r appartmentsMi2Construction, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="Appartment price per m2 vs. year of construction"}
ggplot(data = apartments, aes(construction.year, m2.price)) +
   geom_point(size = 0.3) +
  geom_smooth(se = FALSE, size=1, color = "#371ea3") +
   labs(y="Price per meter-squared", x = "Construction year", title='Appartment price per m2vs. contruction year') + theme_drwhy() + theme(legend.position = "none") 
```

Figure \@ref(fig:appartmentsMi2Surface) indicates a linear relation between *surface* and *m2.price*.

```{r appartmentsMi2Surface, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="Price per meter-squared vs. surface"}
...
```{r appartmentsMi2Norooms, warning=FALSE, message=FALSE, echo=FALSE, fig.width=6, fig.height=4, fig.cap="Price per meter-squared vs. number of rooms"}
