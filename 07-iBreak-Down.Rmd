...
The key function to construct iBD plots is the `DALEX::predict_parts()` function. The use of the function has already been explained in Section \@ref(BDR). In order to perform calcuations that allow obtaining iBD plots, the required argument is `type = "break_down_interactions"`.
...
Interactions are often weak and their net effects are not greater than the contributions of individual variables. If we would like to increase our preference for interactions we can increase the value of the `interaction_preference` argument. The default value of $1$ means no preferences.

```{python,  eval = FALSE}
bd_henry = exp_rf.predict_parts(henry, type='break_down_interactions', interaction_preference = 5)
bd_henry.plot()
```

```{r ibdPython2, echo=FALSE, out.width = '80%', fig.align='center'}
