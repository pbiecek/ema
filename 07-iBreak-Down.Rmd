...
<!---
Table: (\#tab:titanicIBreakDownList) Expected model's predictions $E_{\underline{X}}\{f(\underline{X})|X^i = \underline{x}_*^i, X^j = \underline{x}_*^j\}=\Delta^{\{i,j\}|\emptyset}(\underline{x}_*)+v_0$, paired-variable contributions $\Delta^{\{i,j\}|\emptyset}(\underline{x}_*)$, and interaction contributions $\Delta_{I}^{\{i,j\}}(\underline{x}_*)$ for the random-forest model `titanic_rf_v6` and passenger `johny_d` in the Titanic data.
