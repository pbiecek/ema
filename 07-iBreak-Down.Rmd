...
In Chapter \@ref(breakDown), we presented a model-agnostic approach to calculate of the attribution of an explanatory variable for a model's predictions. However, for some models, like models with interactions, the procedure introduced in Chapter \@ref(breakDown) depends on the ordering of the explanatory variables that are used in computations. 
...
```{r iBreakDownTitanicExamplePlot, warning=FALSE, message=FALSE, echo=FALSE, fig.cap='(ref:iBreakDownTitanicExamplePlotDesc)', out.width = '80%', fig.align='center', fig.width=7, fig.height=4}
library("DALEX")
library("iBreakDown")
...
plot(break_down(explain_titanic_rf, johnny_d, interactions = TRUE))+ theme_ema
```
...
(ref:iBDforHenryDesc) Break-down plot with interactions obtained by applying the generic `plot()` function for the randim-forest model and Henry for the Titanic data.

```{r iBDforHenry, warning=FALSE, message=FALSE, fig.cap='(ref:iBDforHenryDesc)', out.width = '80%', fig.align='center', fig.width=7, fig.height=4}
...
To calculate the attributions with the break down method with interactions one can use the `predict_parts` method with `type='break_down_interactions'` argument. The first argument is the observation for which the attributions are to be calculated. 

Interactions are often weak and their net effects are not greater than the contributions of individual variables. If we would like to increase our preference for interactions we can increase the value of the `interaction_preference` argument. The default value of $1$ means no preferences. Results are stored in the `result` field.

```{python,  eval = FALSE}
bd_henry = titanic_rf_exp.predict_parts(henry, 
                type = 'break_down_interactions', 
                interaction_preference = 5)
bd_henry.result
```
...
```{r ibdPython2, echo=FALSE, out.width = '90%', fig.align='center'}
