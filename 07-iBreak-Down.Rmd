...
```{r iBreakDownTitanicExamplePlot, warning=FALSE, message=FALSE, echo=FALSE, fig.cap="Break-down plot with interactions for the `titanic_rf_v6` model and `johny_d` for the Titanic data.", out.width = '70%', fig.align='center', fig.width=8, fig.height=4}
library("DALEX")
library("iBreakDown")
...
The key function to construct iBD plots is the `DALEX::varaible_attribution()` function that is a wrapper for `iBreakDown` R package [@iBreakDownRPackage]. The use of the function has already been explained in Section \@ref(BDR). The additional necessary argument is `type = "break_down_interactions"`.
...
```{r iBDforHenry, warning=FALSE, message=FALSE, fig.cap="Generic plot() function for the iBreakDown method calculated for `henry`", out.width = '70%', fig.align='center', fig.width=8, fig.height=4}
plot(bd_rf) 
```
