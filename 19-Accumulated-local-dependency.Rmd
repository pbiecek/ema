...
Note that PD, LD, AL profiles describe the effect of a variable in isolation from the values of other variables. In \@ref(eq:trickyModel) model, the effect of variable $X^1$ depends on the value of variable $X^2$. For models with interactions it is subjective to define what would be the "true" main effect of variable $X^1$. But complex predictive models are usually not additive. Examining the case of the \@ref(eq:trickyModel) model, we will gain some intuition on how PD, LS and AL profiles may behave in case of an interaction. 


(ref:accumulatedLocalEffectsNewDesc) Partial-dependence, local-dependence, and accumulated-local profiles for model \@ref(eq:trickyModel). Panel A: ceteris-paribus profiles for 8 selected observations. Panel B: all CP profiles contribute to calculation of PD profile (top), the corresponding partial-dependence profile (bottom). Panel C: only parts of CP profiles close to observations of interest contribute to calculation of LD profile (top), the corresponding local-dependence profile (bottom).  Panel D: only parts of CP profiles contribute to calculation of AL profile (top), the corresponding accumulated-local profile (bottom).

```{r accumulatedLocalEffectsNew, echo=FALSE, fig.cap=, out.width = '90%', fig.align='center', fig.cap='(ref:accumulatedLocalEffectsNewDesc)'}
knitr::include_graphics("figure/CP_ALL.png")
```

The Panel A of Figure \@ref(fig:accumulatedLocalEffectsNew) shows CP profiles for $X^1$ for model \@ref(eq:trickyModel) calculated for selected eight observations. 

The Panel B shows how PD profile for $X^1$ is calculated. It is the average of the CP profiles. Each CP profile is linear and the average value of the $X^2$ variable is chosen so that the average CP profile is zero.
...
Since the $X^1$ and $X^2$ variables are correlated, it can be argued that we should not include entire CP profiles, but only parts of them. In fact, for perfect correlation of explanatory variables, the CP profile for the $i$-th observation should actually be undefined for any values of $z$ different from $x^2_i$. As a consequence, the sum used in the calculation of the PD profile in equation \@ref(eq:PDtrickyModel) would involve undefined terms for any $z$.
...
The Panel C of Figure \@ref(fig:accumulatedLocalEffectsNew) shows LD profile would look like for empirical estimate of the marginal distribution calculated in four bins. We can also use the theoretical (here known) distribution. The conditional distribution of $X^2$, given $X^1=z$, is just a probability mass of 1 at $z$. Consequently, for model \@ref(eq:trickyModel), the LD profile for $X^1$ and any $z \in [-1,1]$ is given by
...
The LD profile shows the average response over the conditional distribution. Part of which can be attributed to the effect of the correlated variable $X^2$. AL profile shows the net variability of $X^1$ variable itself. The Panel D of Figure \@ref(fig:accumulatedLocalEffectsNew) shows AL profile would look like for empirical estimate of the marginal distribution calculated in four bins.  Since we know the joint distribution of explanatory variables and the formula for the model we explicitly calculate the AL profile. 
...
It is clear that PD, LD and AL profiles show different aspects of the model. In the analyzed example for tricky model we got three different explanations of X1 variable effect. 

In practice, the explanatory variables are typically correlated and complex predictive models are usually not additive. Therefore, when analyzing the model it is worth to check how much PD, LD and AL models differ. And if so, look for potential causes. Correlations can be detected at the stage of data exploration. Interactions can be noticed by looking at individual CP profiles.
