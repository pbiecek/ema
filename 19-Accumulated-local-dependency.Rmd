...
where $z_0$ if the lower boundary  of $x^j$. The profile $g_{AL}^{f, j}(z)$ is calculated up to some constant $c$. Usually the constant $c$ is selected to set average $g_{AL}^{f, j}$ equal to 0 or an average of $f(x)$.
...
\begin{equation}
\hat g^{ALE}_i(z) =
\sum_{k=1}^{k_i(z)} \frac{1}{|N_i(k)|}
\sum_{j \in N_i} [ f(x|^i = z_k) - f(x|^i = z_{k-1]) ] + c
(\#eq:ALEPest)
\end{equation}
...
\begin{equation}
\tilde g^{ALE}_i(z) = \sum_{k \in {z_0, ..., z}} \frac{j = 1}{n}  w_{j}(k) [f(x|^i = z_k) - f(x|^i = z_k - \Delta)] + c
(\#eq:ALEPest2)
\end{equation}
