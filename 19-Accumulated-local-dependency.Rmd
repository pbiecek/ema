...
pd_rf <- variable_profile(explain_apartments_rf, variables = c("no.rooms", "surface"), type = "partial")
ac_rf <- variable_profile(explain_apartments_rf, variables = c("no.rooms", "surface"), type = "accumulated")
cd_rf <- variable_profile(explain_apartments_rf, variables = c("no.rooms", "surface"), type = "conditional")

pd_rf$agr_profiles$`_label_` <- "RF partial dependency"
ac_rf$agr_profiles$`_label_` <- "RF accumulated dependency"
cd_rf$agr_profiles$`_label_` <- "RF conditional dependency"

plot(pd_rf$agr_profiles, ac_rf$agr_profiles, cd_rf$agr_profiles) + 
  ylab("feature effect") +
...
Here we show partial dependency profiles calculated with `DALEX` package that wraps functions from the  `ingredients` package [@ingredientsRPackage]. You will also find similar functions in the `ALEPlots` package [@ALEPlotRPackage].

All profile functions can be calculated with the function `DALEX::variable_profile` with specified parameter `type`.

Partial dependency profiles can be calculated with `type = "partial"`.
Conditional dependency profiles can be calculated with `type = "conditional"`.
Accumulated local profiles can be calculated with `type = "accumulated"`.
...
pd_rf <- variable_profile(explain_apartments_rf, variables = c("no.rooms", "surface"))
...
ac_rf <- variable_profile(explain_apartments_rf, variables = c("no.rooms", "surface"),
                          type = "accumulated")
...
cd_rf <- variable_profile(explain_apartments_rf, variables = c("no.rooms", "surface"),
                          type = "conditional")
plot(cd_rf) + ylab("Conditional dependency") +
