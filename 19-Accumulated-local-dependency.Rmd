...
& \ \ \ \ \ 228\times 0.80 + 204\times 1.00)/1000  \nonumber \\
& = 0.6.
(\#eq:fullDataPD)
\end{align}
...
&= \frac{1}{1000}(198\times 0.19+197\times 0.59+ 173\times 0.99 + \nonumber \\
& \ \ \ \ \ 228\times 1.40 + 204\times 1.77)=1.  
\end{align}
...
In fact, by using the same extrapolation principle, we get $\hat g_{PD}^{1}(z) = 1$ also for $z \in (0.2,0.4]$, $(0.4,0.6]$, $(0.6,0.8]$, and $(0.8,1]$. Thus, the obtained profile indicates no effect of $X^1$, which is clearly a wrong conclusion. 
...
[TOMASZ: I STILL THINK THAT THIS EXAMPLE IS CONFUSING, AS IT MIXES EFFECTS OF CORRELATION AND SAMPLE MEAN OF 0 (WHICH ARE ESTIMATION ISSUES), AND INTERACTION (WHICH IS A PROFILE-DEFINITION ISSUE).]

In this section, we illustrate in more detail the behavior of PD, LD, and AL profiles for a model with an interaction and correlated explanatory variables. In particular, let us consider the following, simple model with for two explanatory variables: 
...
[TOMASZ: THE KEY QUESTION IS: WHAT IS THE "CORRECT" SUMMARY OF THE EFFECT OF $X^1$ IN THIS MODEL? IT IS NEVER STATED, SO IT IS DIFFICULT TO SEE BIAS IN DIFFERENT PROFILES. BASED ON THE MODEL FUNCTION, ONE COULD ARGUE THAT THE EFFECT IS LINEAR, BUT WITH A SLOPE DEPENDING ON X2.]
...
(ref:accumulatedLocalEffectsNewDesc) Partial-dependence, local-dependence, and accumulated-local profiles for a simple model with interaction and correlated explanatory variables. Panel A: ceteris-paribus profiles (top plot) and the corresponding partial-dependence profile. Panel B: local-dependence profile. Panel C: accumulated-local profile. [TOMASZ: REMOVE THE LEFT PANEL WITH CP PROFILES ONLY. THE FIGURE IS ONLY USED FOR PD. IT CAN BE SKIPPED, IN FACT, AS EVERYTHING IS BEING EXPLAINED DIRECTLY.]

```{r accumulatedLocalEffectsNew, echo=FALSE, fig.cap=, out.width = '90%', fig.align='center', fig.cap='(ref:accumulatedLocalEffectsNewDesc)'}
knitr::include_graphics("figure/CP_ALL.png")
```

The top part of Panel A of Figure \@ref(fig:accumulatedLocalEffectsNew) shows CP profiles for $X^1$ for model \@ref(eq:trickyModel) calculated for selected eight observations. The bottom part of the panel presents the corresponding estimate of the PD profile for $X^1$, i.e., the average of the CP profiles. The profile suggests no effect of variable $X^1$, which is clearly a misleading conclusion. [TOMASZ: THE FIGURE IS ONLY USED FOR PD. IT CAN BE SKIPPED, IN FACT, AS EVERYTHING IS BEING EXPLAINED DIRECTLY.]
...
As a result, the PD profile for $X^1$ is estimated as a horizontal line at 0, as seen in the bottom part of Panel A of Figure \@ref(fig:accumulatedLocalEffectsNew). [TOMASZ: BUT IF THE SAMPLE MEAN OF $X^1$ WERE NOT 0, THE PD PROFILE WOULD BE A LINEAR FUNCTION OF Z. AND ONE COULD SAY THAT IT WOULD CORRECTLY (THOUGH WITH A "WRONG" SLOPE) REFLECT THE LINEARITY OF THE CP PROFILES. THE READER COULD ASK: WOULD THAT BE WRONG? AND USING 0 CAN BE SEEN AS CREATING AN ARTIFICIAL EFFECT, FORCING THE PD PROFILE TO LOOK "BAD".]
...
Thus, the issue is related to the fact of using the marginal distribution of $X^2$, which disregards the value of $X^1$, in the definition of the PD profile. This observation suggests a modification: instead of the marginal distribution, one might consider the conditional distribution of $X^2$ given $X^1$. The modification leads to the definition of LD profile. 

In our example, the conditional distribution of $X^2$, given $X^1=z$, is just a probability mass of 1 at $z$. Consequently, for model \@ref(eq:trickyModel), the LD profile for $X^1$ and any $z \in [-1,1]$ is given by
...
[TOMASZ: SO, THIS IS THE MODEL FUNCTION. THE READER COULD ASK: WHAT IS WRONG WITH THAT?] It turns out, however, that the modification does not fully address the issue of correlated explanatory variables. [TOMASZ: THIS IS CONFUSING. CORRELATION IS AN ESTIMATION ISSUE. THE FACT THAT CORRELATION DOES SOMETHING TO THE ESTIMATION OF THE LD PROFILE DOES NOT MEAN THAT THE PROFILE IS ACTUALLY WRONG.] In particular, equation \@ref(eq:trickyModel) indicates that the effect of $X^1$ is similar to function $f(x^1) = -(x^1+1)$ for values close to $(x^1, x^2) = (-1,-1)$ and similar to function $f(x^1) = x^1 +1$ for values close to $(x^1, x^2) = (1,1)$. Thus, it linearly decreases at the rate equal to 1 for values close to $(-1,-1)$ and increases with the same rate for values close to $(1,1)$. The function given in \@ref(eq:LDtrickyModel) does not explicitly show this. 
...
This formula better reflects the behavior of function $f(X^1,X^2)$ due to $X^1$ removing the effect of changes in variable $X^2$. [TOMASZ: I DO NOT UNDERSTAND THIS SENTENCE. AND: THE OBTAINED AL IS A QUADRATIC FUNCTION OF Z, AS IS THE LD PROFILE. SO WHY IS AL OK, BUT LD IS WRONG? IN FACT, ONE COULD ARGUE THAT BOTH ARE WRONG, WHILE THE LINEAR PD (IF IT THE SAMPLE MEAN WERE NOT CHOSEN TO BE 0) WOULD BE APPROXIMATELY RIGHT.]
...
ld_rf.result
```
[TOMASZ: RESULTS AS A FIGURE? COMMENTS WERE USED IN CHAP. 4]
```{r examplePythonLDProfile1, echo=FALSE, out.width = '100%', fig.align='center'}
